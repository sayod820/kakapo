...
export function createRecoveryHttpAdapter(opts = {}) {
  ...
  if (isProdHost) {
    // Always refuse unless BOTH the legacy latch and PC-4B operator gates pass.
    if (!allowProductionHost) {
      throw new Error('REFUSE_PRODUCTION_HOST: adapter blocked from production mutation hosts')
    }
    if (opts.productionReplayGate) {
      const gate = opts.productionReplayGate
      if (!gate || gate.ok !== true || gate.code !== 'PRODUCTION_REPLAY_ENABLED') {
        throw new Error('REFUSE_PRODUCTION_REPLAY: operator production gates not satisfied')
      }
      if (String(gate.baseUrl || '').replace(/\/$/, '') !== baseUrl) {
        throw new Error('REFUSE_PRODUCTION_REPLAY: gate baseUrl mismatch')
      }
    } else if (opts.requireProductionReplayGate !== false) {
      // Default: even with allowProductionHost, require explicit gate object.
  ...
  async function http(method, path, body, meta = {}) {
    ...
    if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) state.posts += 1
