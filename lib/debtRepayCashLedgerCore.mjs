 ...
 * Never copies debtRepayCash across different server shift ids (closed → new open).
 * Allows off-* → server adopt so pending local repay survives remap.
 */
export function withPreservedDebtRepayCash(localShift, remoteShift) {
  const remoteId = String(remoteShift?.id || '').trim()
  const localId = String(localShift?.id || '').trim()
  const shiftId = remoteId || localId
  const sameShift = Boolean(localId && shiftId && localId === shiftId)
  const orphanAdopt = Boolean(localId && remoteId && isOffShiftId(localId) && !isOffShiftId(remoteId))
  const fromLocal = (sameShift || orphanAdopt)
    ? round2(Number(localShift?.debtRepayCash) || 0)
    : 0
