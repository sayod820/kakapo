...
export function queueWatermark(queue) {
  ...
  const fingerprint = sha256Hex(refs.map((ref, i) => {
...
export function kindSafetyMatrix() {
  ...
  return [
    ...
    row('stock_layer_update', { replaySupported: true, idempotencyProven: true }),
    row('stock_layer_delete', { replaySupported: true, idempotencyProven: true }),
...
export function executeShiftRemapBatch(session, rows, world) {
  ...
  for (const plan of rows || []) {
    ...
    // Prior SHIFT_CLOSED failures become stale after explicit remap onto open target
    row.failed = false
    row.lastError = ''
...
async function sendOpViaAdapter(api, row) {
  ...
  if (kind === 'stock_layer_delete' && api.deleteStockLayer) {
    return api.deleteStockLayer({ ...row.payload, clientRef: row.clientRef })
  }
...
export async function executeRecoveryReplay(session, world, api, options = {}) {
  ...
  while (processed < limit) {
    ...
    try {
      ...
      const msg = String(e?.message || e)
      const recoveryClass = String(e?.recoveryClass || e?.recovery?.class || '')
      const isTimeoutLike = /timeout|CRASH_AFTER_POST|lost_response|ECONN|fetch failed|network|abort/i.test(msg)
        || /TIMEOUT_AFTER_COMMIT|TIMEOUT_BEFORE_COMMIT/i.test(recoveryClass)
      if (isTimeoutLike) {
        ...
        // No server proof yet — pause/resume same clientRef (before-commit network loss)
        if (/TIMEOUT_BEFORE_COMMIT/i.test(recoveryClass) || /fetch failed|ECONN|abort|network/i.test(msg)) {
          s = persistSessionPatch(s, {
            status: SESSION_STATUS.PAUSED,
            stoppedReason: `timeout_before_commit:${row.clientRef}`,
          })
          return {
            ok: false,
            error: 'timeout_before_commit',
            resumable: true,
            clientRef: row.clientRef,
            session: s,
            world,
        ...
        return { ok: false, error: 'timeout_unconfirmed', resumable: true, clientRef: row.clientRef, session: s, world, processed }
      }
...
export function verifyPostDrain(session, world, opts = {}) {
  ...
  const errors = []
  if (queue.length > 0) errors.push(`queue_not_empty:${queue.length}`)
  ...
  if (classified.ops.some(o => o.classification === CLASS.DEPENDENCY_BLOCKED)) {
    errors.push('dependency_blocked_remaining')
  }
