  ...
  const items = Array.isArray(p.items)
    ? p.items.map(it => ({
      ...
      name: it.name != null ? String(it.name)
        : (it.productName != null ? String(it.productName) : undefined),
    }))
    : []
  const core = {
    kind: String(kind || ''),
    ...
    // Do NOT fingerprint raw `total` — Desktop queue often omits it while server
    // includes derived total; payments + items already prove business identity.
    ...
    // Destructive stock identity (stock_layer_delete)
    receiptId: p.receiptId != null ? String(p.receiptId) : undefined,
    productId: p.productId != null ? String(p.productId) : undefined,
  }
...
export function classifyPendingOpFresh(row, ctx = {}) {
  ...
  if (row.failed) {
    ...
    if (/SHIFT_CLOSED|смена уже закрыта|SHIFT_NOT_FOUND|смена не найдена/i.test(err)) {
      ...
      const stillClosed = (ctx.serverClosedIds || new Set()).has(shiftId)
        || (sh && String(sh.status) !== 'open')
      // After explicit recovery remap onto an open target, prior SHIFT_CLOSED is stale
      if (shiftId && !stillClosed) {
    ...
    } else if (/не найден|BrokenRef|ждём|синтетический/i.test(err)) {
