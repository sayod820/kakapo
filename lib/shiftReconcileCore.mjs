...
/**
 * Закрытие смены на кассе окончательно: входящий «open» с сервера (закрытие ещё в очереди)
 * не должен оживлять смену — иначе утром касса продолжает вчерашнюю смену.
 */
export function protectLocallyClosedShifts(localShifts, nextShifts) {
  const closedLocal = new Map()
  for (const sh of localShifts || []) {
    if (String(sh?.status || '') === 'closed' && sh?.id) closedLocal.set(String(sh.id), sh)
  }
  if (!closedLocal.size) return nextShifts
  let changed = false
  const out = (nextShifts || []).map(sh => {
    if (String(sh?.status || '') !== 'open') return sh
    const local = closedLocal.get(String(sh?.id || ''))
    if (!local) return sh
    changed = true
    const row = { ...sh, status: 'closed' }
    for (const k of CLOSED_SHIFT_FIELDS) {
      if (local[k] !== undefined) row[k] = local[k]
    }
    return row
  })
  return changed ? out : nextShifts
}

/**
 * Закрытие на сервере тоже окончательно: локальная «open» с более свежим updatedAtIso
 * (утренние чеки на кассе) не должна перебивать серверное «closed» по LWW.
 */
export function adoptServerClosedShifts(nextShifts, remoteShifts) {
  const byId = new Map()
  const byRef = new Map()
  for (const sh of remoteShifts || []) {
    if (String(sh?.status || '') !== 'closed') continue
    if (sh?.id) byId.set(String(sh.id), sh)
    if (sh?.clientRef) byRef.set(String(sh.clientRef), sh)
  }
  if (!byId.size) return nextShifts
  let changed = false
  const out = (nextShifts || []).map(sh => {
    if (String(sh?.status || '') !== 'open') return sh
    const remote = byId.get(String(sh?.id || '')) || (sh?.clientRef ? byRef.get(String(sh.clientRef)) : undefined)
    if (!remote) return sh
    changed = true
    const row = { ...sh, status: 'closed' }
    for (const k of CLOSED_SHIFT_FIELDS) {
      if (remote[k] !== undefined) row[k] = remote[k]
    }
    return row
  })
  return changed ? out : nextShifts
