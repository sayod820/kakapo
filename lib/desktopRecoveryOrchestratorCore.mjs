...
/**
 * Recovery armed earlier (e.g. by SHIFT_CLOSED / off- shift markers, no longer triggers)
 * but never touched the server: safe to leave it and let normal flush deliver the queue.
 */
export function canAutoExitStaleRecovery(input = {}) {
  const queue = input.queue || []
  const meta = input.meta || {}
  const idem = queue.filter(r => r?.failed && /IDEMPOTENCY_KEY_REUSED|тот же clientRef/i.test(String(r.lastError || '')))
  if (idem.length) return { ok: false, reason: 'idempotency_failed' }
  const sess = meta.recoverySession && typeof meta.recoverySession === 'object' ? meta.recoverySession : null
  if (sess) {
    if (sess.targetServerShiftId) return { ok: false, reason: 'recovery_shift_opened' }
    if (String(sess.phase || '') === 'RECOVERY_REPLAY') return { ok: false, reason: 'replay_started' }
    if ((sess.completedClientRefs || []).length) return { ok: false, reason: 'replay_progress' }
    if ((sess.remappedClientRefs || []).length) return { ok: false, reason: 'remapped' }
  }
  if (String(meta.recoveryPhase || '') === 'RECOVERY_REPLAY') return { ok: false, reason: 'replay_phase' }
  return { ok: true }
}
