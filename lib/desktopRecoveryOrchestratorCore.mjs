...
export function detectRecoveryNeed(input = {}) {
  ...
  if (shiftFail.length) reasons.push(`shift_failed:${shiftFail.length}`)

  const ghostSales = queue.filter(r =>
    r?.kind === 'sale' && /^off-/i.test(String(r?.payload?.shiftId || r?.shiftId || '')))
  if (ghostSales.length) reasons.push(`ghost_shift_sales:${ghostSales.length}`)

  // Version bump with legacy-shaped pending queue (not every normal offline sale)
  const verChanged = input.appVersion
    && input.previousAppVersion
    && String(input.appVersion) !== String(input.previousAppVersion)
  if (verChanged && queue.length > 0 && (idem.length || shiftFail.length || ghostSales.length)) {
