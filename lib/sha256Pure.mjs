/**
 * Minimal SHA-256 (browser + Node safe). No node:crypto import —
 * required so Desktop Next client bundle can import recovery cores.
 */
...
export function sha256Hex(message) {
  ...
  const withPad = l + 1
  const padLen = (withPad % 64 <= 56) ? 56 - (withPad % 64) : 120 - (withPad % 64)
