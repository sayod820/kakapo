...
function payLabel(sale: PosSale) {
  ...
  if (sale.paymentMethod === 'wallet') return 'Бонусами'
  if ((Number(sale.paidWallet) || 0) > 0.001) return 'Бонусами'
...
export function buildPosReceiptHtml(sale: PosSale, opts?: PosReceiptPrintOpts): string {
  ...
  const itemHtml = tpl.showItems ? items.map((it, idx) => {
    ...
    let name = rawName
    let detail = pack
    if (!detail && rawName.length > maxLeft && vol && vol[1].trim().length >= 8 && vol[1].trim().length <= maxLeft) {
      name = vol[1].trim()
      detail = vol[2].trim()
    } else if (!detail && vol && vol[1].trim().length >= 3) {
      name = vol[1].trim()
      detail = vol[2].trim()
    }
  ...
  const payments = [
    ...
    (Number(sale.paidWallet) || 0) > 0.001 ? moneyRow('Бонусами', Number(sale.paidWallet) || 0, { currency: cur }) : '',
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .item-was{font-size:12px;color:#000}
  .item-was .strike{text-decoration:line-through;text-decoration-thickness:1.5px}
  .item-now{font-size:12px;font-weight:800}
