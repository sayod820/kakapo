...
test('T4 projection closes orphans, remaps sales, never deletes sales', () => {
  ...
  // Counters recomputed from unique sale rows after remap — never max(ghost, server)
  expect(auth?.salesCount === 2, `count=${auth?.salesCount} (remapped+existing, not max 142/69)`)
  expect(auth?.salesCash === 0, 'cash from sale rows without paidCash fields → 0')
