...
const LOGIN_CSS = `
  ...
  .cl-inp:focus{border-color:rgba(31,215,96,.65)!important;box-shadow:0 0 0 3px rgba(31,215,96,.15)!important;outline:none;}
  ...
  @keyframes clGlow{0%,100%{box-shadow:0 0 28px rgba(31,215,96,.35),0 0 60px rgba(31,215,96,.12)}50%{box-shadow:0 0 40px rgba(31,215,96,.55),0 0 80px rgba(31,215,96,.2)}}
        ...
        <div style={{ textAlign: 'center', marginBottom: 22, animation: 'clFadeUp .55s ease both' }}>
          <div style={{ position: 'relative', width: 72, height: 72, margin: '0 auto 14px' }}>
            <div style={{
              ...
              background: 'linear-gradient(145deg,#0F8A3A 0%,#1FD760 50%,#5FE88A 100%)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ fontSize: 12, color: '#8FB897' }}>
            Доставка · <span style={{ color: '#1FD760', fontWeight: 700 }}>г. Яван</span>
          </div>
        </div>
            ...
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{
                ...
                background: step === s || (step === 'otp' && s === 'phone')
                  ? 'linear-gradient(135deg,#0F8A3A,#1FD760)' : '#0C1C0F',
                color: step === s || (step === 'otp' && s === 'phone') ? '#030B05' : '#3D6645',
                border: `1px solid ${step === s ? 'rgba(31,215,96,.6)' : '#162B1A'}`,
              }}>{i + 1}</div>
              <span style={{ fontSize: 10, fontWeight: 700, color: step === s ? '#1FD760' : '#3D6645' }}>
                {s === 'phone' ? 'Телефон' : 'Код'}
              </span>
              {i === 0 && <div style={{ width: 18, height: 2, borderRadius: 1, background: step === 'otp' ? '#1FD760' : '#162B1A', marginLeft: 2 }} />}
            </div>
          ))}
        </div>
          ...
          {step === 'phone' ? (
            <div key="phone-step">
                    ...
                    {demoList.map(c => (
                      <button
                        ...
                        style={{
                          ...
                          background: phone === c.phone ? 'rgba(31,215,96,.14)' : 'rgba(31,215,96,.06)',
                          border: `1.5px solid ${phone === c.phone ? 'rgba(31,215,96,.45)' : 'rgba(31,215,96,.15)'}`,
                        }}
                      >
                        <div style={{
              ...
              <button type="button" onClick={sendOtp} disabled={load} className="cl-btn cl-ub"
                style={{
                  ...
                  background: 'linear-gradient(135deg,#0F8A3A 0%,#1FD760 50%,#5FE88A 100%)',
                  border: 'none', color: '#030B05', fontWeight: 800, fontSize: 13,
            ...
            <div key="otp-step">
              ...
              <div style={{ fontSize: 11, color: '#8FB897', textAlign: 'center', marginBottom: 16, lineHeight: 1.45 }}>
                Код на <span style={{ color: '#1FD760', fontWeight: 800 }}>{phone}</span>
              </div>
                ...
                {otp.map((v, i) => (
                  <input
                    ...
                    style={{
                      ...
                      boxShadow: focusedOtp === i ? '0 0 16px rgba(31,215,96,.2)' : 'none',
              ...
              <div style={{
                ...
                <div style={{ fontSize: 11, color: '#8FB897' }}>
                  Демо: <span className="cl-ub" style={{ color: '#1FD760', fontWeight: 900, letterSpacing: 2 }}>1234</span>
                </div>
                {cd > 0 && (
              ...
              <button type="button" onClick={verify} disabled={load || !otpReady} className="cl-btn cl-ub"
                style={{
                  ...
                  border: 'none', color: '#030B05',
                  fontWeight: 800, fontSize: 13,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  boxShadow: '0 6px 22px rgba(31,215,96,.3)',
