...
import PosPointsAdminPage from '@/components/admin/PosPointsAdminPage'
import {
...
function AdminLocationMap({
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 6, fontWeight: 700 }}>📍 Точка на карте *</div>
      {!mounted ? (
        <div style={{ height: 280, borderRadius: 14, background: 'var(--l1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--b1)' }}>
...
const CSS = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .admin-shell[data-theme="light"] .ac{
    box-shadow:0 1px 4px rgba(12,26,16,.06);
  }
...
/* ── HELPERS ─────────────────────────────────────── */
const SC_STATUS = {new:{l:'Новый',c:'#FF4545'},assembling:{l:'Собирается',c:'#9B6DFF'},cooking:{l:'Готовится',c:'#FF8C00'},delivering:{l:'В пути',c:'#3B8EF0'},delivered:{l:'Доставлен',c:'#1FD760'},cancelled:{l:'Отменён',c:'var(--t3)'}};
const LVC = {basic:'var(--t2)',bronze:'#CD7F32',silver:'#C0C0C0',gold:'#FFB800',platinum:'#3B8EF0'};

const Tog = ({on,set}) => (
  <div onClick={() => { if (typeof set === 'function') set(); }} style={{width:44,height:24,borderRadius:12,background:on?'#1FD760':'var(--b2)',position:'relative',cursor:'pointer',transition:'background .2s',flexShrink:0}}>
...
const StatCard = ({l,v,c,e,sub,onClick,active}) => (
  <button
    ...
    style={{
      ...
      border:active?`1.5px solid ${c||'rgba(31,215,96,.45)'}`:'1px solid var(--b1)',
      background:active?'rgba(31,215,96,.08)':'var(--l2)',
  ...
  >
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
      <div style={{fontSize:11,color:'var(--t2)',fontWeight:600}}>{l}</div>
      {e&&<span style={{fontSize:22}}>{e}</span>}
    </div>
    <div className="ub" style={{fontSize:22,fontWeight:900,color:c||'var(--t1)'}}>{v}</div>
...
const NI = ({lbl,val,set,ph='',type='text',suf=''}) => (
  <div>
    ...
    <div style={{position:'relative'}}>
      ...
      {suf&&<span style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',fontSize:11,color:'var(--t3)',fontWeight:700}}>{suf}</span>}
    </div>
  </div>
);
...
const DebtReadOnly = ({ debt, onManage }: { debt: number; onManage?: () => void }) => (
  <div>
    <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 5, fontWeight: 700 }}>Долг ЅМ</div>
    <div style={{
      padding: '9px 13px', borderRadius: 10, background: 'var(--l2)', border: '1.5px solid var(--b1)',
      display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap',
    }}>
      <span className="ub" style={{ fontSize: 15, fontWeight: 900, color: debt > 0 ? '#FF4545' : 'var(--t2)' }}>
    ...
    <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 6, lineHeight: 1.45 }}>
  ...
  return (
    ...
    >
      {ADMIN_STATUS_OPTIONS.map(st => {
        ...
        return <option key={st} value={st} style={{ background: 'var(--l3)', color: 'var(--t1)' }}>{lb.l}</option>
      })}
    </select>
  )
}
...
function OrderPersonSelect({ value, options, onChange, disabled = false, accent = 'var(--t2)' }) {
  const hasValue = !!value
  const color = hasValue ? accent : 'var(--t3)'
  return (
    <select
      value={value}
      disabled={disabled}
    ...
    >
      <option value="" style={{ background: 'var(--l3)', color: 'var(--t1)' }}>—</option>
      {options.map(p => (
        <option key={p.id} value={p.id} style={{ background: 'var(--l3)', color: 'var(--t1)' }}>{p.name}</option>
      ))}
    </select>
  )
}
    ...
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.72)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} className="ac" style={{ width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', padding:22 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18 }}>
          ...
          <button onClick={onClose} className="btn" style={{ width:32, height:32, borderRadius:10, background:'var(--l3)', border:'1px solid var(--b1)', color:'var(--t2)', fontSize:16 }}>✕</button>
        </div>

        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
          <div style={{ background:'var(--l3)', borderRadius:12, padding:12, border:'1px solid var(--b1)' }}>
            <div style={{ fontSize:10, color:'var(--t3)', marginBottom:6, fontWeight:700 }}>КЛИЕНТ</div>
            <div style={{ fontWeight:700, fontSize:13, marginBottom:2 }}>{order.client}</div>
            <div style={{ fontSize:11, color:'var(--t2)' }}>{order.phone}</div>
            <div style={{ fontSize:11, color:'var(--t2)', marginTop:4 }}>📍 {order.addr || '—'}</div>
          </div>
          <div style={{ background:'var(--l3)', borderRadius:12, padding:12, border:'1px solid var(--b1)' }}>
            <div style={{ fontSize:10, color:'var(--t3)', marginBottom:6, fontWeight:700 }}>КОМАНДА</div>
            {showAssembler && (
            <div style={{ fontSize:12, marginBottom:8 }}>
              <div style={{ fontSize:10, color:'var(--t3)', marginBottom:4 }}>Сборщик</div>
              {onAssemblerChange ? (
                <OrderPersonSelect
            ...
            <div style={{ fontSize:12 }}>
              ...
              {onCourierChange ? (
                ...
                <>
                  ...
                  {order.courierPhone && <span style={{ color:'var(--t3)', fontSize:10 }}> · {order.courierPhone}</span>}
                </>
              )}
            </div>
          </div>
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:10, color:'var(--t3)', marginBottom:8, fontWeight:700 }}>СОСТАВ ЗАКАЗА</div>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {(order.itemsDetailed || []).map((it, i) => (
              <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 10px', background:'var(--l3)', borderRadius:10, border:'1px solid var(--b1)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:10, color:'var(--t3)' }}>{it.qty} шт · {it.source === 'restaurant' ? 'ресторан' : 'магазин'}</div>
                  </div>
                </div>
        ...
        <div style={{ marginTop: 14, padding: '12px 14px', background: 'var(--l3)', borderRadius: 12, border: '1px solid var(--b1)' }}>
          ...
          {(deliveryFee > 0 || bonusSpent > 0) && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 14, paddingTop: 8, borderTop: '1px solid var(--b1)' }}>
              <span style={{ color: 'var(--t1)', fontWeight: 700 }}>К оплате</span>
          ...
          {order.comment && <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 4 }}>💬 {order.comment}</div>}
        </div>

        {onStatusChange && (
          <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid var(--b1)' }}>
            <div style={{ fontSize:10, color:'var(--t3)', marginBottom:8, fontWeight:700 }}>ИЗМЕНИТЬ СТАТУС</div>
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10,marginBottom:18}}>
        {['all','new','assembling','delivering','delivered','cancelled'].map(s=>{
          ...
          const sc  = s==='all' ? {l:'Все',c:'var(--t1)'} : s==='assembling' ? {l:'В работе',c:'#9B6DFF'} : adminStatusLabel(s);
          return (
            <div key={s} onClick={()=>setStatus(s)} className="ac" style={{padding:'12px 14px',cursor:'pointer',border:`1.5px solid ${status===s?'rgba(31,215,96,.3)':'var(--b1)'}`,background:status===s?'rgba(31,215,96,.06)':'var(--l2)'}}>
              <div style={{fontSize:10,color:'var(--t2)',marginBottom:5}}>{s==='all'?'Все заказы':sc.l}</div>
      ...
      <div style={{display:'flex',gap:10,marginBottom:14,alignItems:'center',flexWrap:'wrap'}}>
        <div style={{position:'relative',flex:1,minWidth:200,maxWidth:280}}>
          ...
          {orderSearch && (
            <button
              ...
              className="btn"
              style={{position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',background:'transparent',border:'none',color:'var(--t3)',fontSize:16,padding:4,lineHeight:1}}
              aria-label="Очистить"
            >×</button>
          )}
        </div>
        {[{id:'all',l:'Все типы'},{id:'market',l:'🛒 Магазин'},{id:'restaurant',l:'🍽 Рестораны'},{id:'mixed',l:'🔀 Смешанные'}].map(f=>(
          <button key={f.id} onClick={()=>setType(f.id)} className="ab"
            style={{padding:'7px 14px',fontSize:12,background:type===f.id?'rgba(31,215,96,.12)':'var(--l3)',border:`1.5px solid ${type===f.id?'rgba(31,215,96,.35)':'var(--b1)'}`,color:type===f.id?'#1FD760':'var(--t2)'}}>
            {f.l}
          </button>
        ))}
        {orderSearch.trim() && (
        ...
        <div style={{
          ...
          <select
            ...
            style={{
              ...
              fontWeight:700,
              background:'var(--l3)',
              color: bulkStatus ? adminStatusLabel(bulkStatus).c : 'var(--t2)',
              border:`1px solid ${bulkStatus ? `${adminStatusLabel(bulkStatus).c}40` : 'var(--b1)'}`,
              cursor: bulkDeleting || bulkStatusChanging ? 'default' : 'pointer',
              minWidth: 150,
              opacity: bulkDeleting || bulkStatusChanging ? .6 : 1,
            }}
          >
            <option value="" style={{ background:'var(--l3)', color:'var(--t2)' }}>Новый статус…</option>
            {ADMIN_STATUS_OPTIONS.map(st => {
              const lb = adminStatusLabel(st);
              return <option key={st} value={st} style={{ background:'var(--l3)', color:'var(--t1)' }}>{lb.l}</option>;
            })}
          ...
          <button
            ...
            disabled={bulkDeleting || bulkStatusChanging}
            className="ab"
            style={{ padding:'6px 14px', fontSize:12, background:'var(--l3)', border:'1px solid var(--b1)', color:'var(--t2)', marginLeft:'auto' }}
          >
            Снять выбор
          </button>
        </div>
      ...
      <div className="ac">
        <table className="at">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={12} style={{textAlign:'center',padding:'28px 14px',color:'var(--t2)',fontSize:13}}>
                  {orderSearch.trim()
                ...
                <tr key={o.id} onClick={()=>setDetail(o)} style={{ cursor:'pointer' }}>
                  ...
                  <td><div style={{fontWeight:600,fontSize:12}}>{o.client}</div><div style={{fontSize:10,color:'var(--t3)'}}>{o.phone}</div></td>
                  <td style={{fontSize:10,color:'var(--t2)',maxWidth:100}}>{o.addr||'—'}</td>
                  <td style={{fontSize:11,color:'var(--t2)',maxWidth:130}}>{o.items}</td>
                  <td><span className="ub" style={{fontSize:12,fontWeight:800}}>{o.total} ЅМ</span></td>
                  <td style={{fontSize:11,color:o.courier==='—'?'var(--t3)':'#3B8EF0',fontWeight:o.courier==='—'?400:700}}>{o.courier}</td>
                  <td style={{fontSize:11,color:o.assembler==='—'?'var(--t3)':'#9B6DFF',fontWeight:o.assembler==='—'?400:700}}>{o.assembler}</td>
                  <td>
                    {(() => { const sc = adminStatusLabel(o.status); return <Badge v={sc.l} c={sc.c}/>; })()}
                  </td>
                  <td style={{fontSize:11,color:'var(--t3)'}}>{o.time}</td>
                  <td onClick={e => e.stopPropagation()}>
                    <button
                      type="button"
                      title="Удалить заказ"
...
function BulkPricingFields({ tiers, onChange, sellType }) {
  ...
  return (
    <div style={{ padding:'12px 14px', borderRadius:12, background:'var(--l3)', border:'1px solid var(--b1)' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
        <div style={{ fontSize:11, color:'var(--t2)', fontWeight:700 }}>📦 Оптовые цены</div>
        <button type="button" onClick={() => onChange([...rows, { minQty: '', price: '' }])} className="ab abg" style={{ padding:'4px 10px', fontSize:11 }}>+ Уровень</button>
      </div>
      <div style={{ fontSize:10, color:'var(--t3)', marginBottom:10, lineHeight:1.45 }}>
        При достижении количества цена за {sellType === 'weight' ? 'порцию' : 'шт'} меняется для всей позиции. Пример: кекс — от 24 шт по 1.8 ЅМ.
      </div>
      {!rows.length ? (
        <div style={{ fontSize:11, color:'var(--t3)' }}>Одна розничная цена. Нажмите «+ Уровень» для опта.</div>
      ) : rows.map((row, i) => (
        <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:8, marginBottom:8 }}>
          <div>
            <div style={{ fontSize:10, color:'var(--t3)', marginBottom:4 }}>От ({unit})</div>
            <input className="ai" type="number" value={row.minQty} onChange={e => setRow(i, { minQty: e.target.value })} placeholder="24" />
          </div>
          <div>
            <div style={{ fontSize:10, color:'var(--t3)', marginBottom:4 }}>Цена (ЅМ)</div>
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:18, overflowX:'auto', paddingBottom:4 }}>
        <button type="button" onClick={()=>setCatFlt('all')} className="ab" style={{ flexShrink:0, minWidth:72, padding:'10px 12px', borderRadius:12, textAlign:'center', background:catFlt==='all'?'rgba(31,215,96,.14)':'var(--l2)', border:`1.5px solid ${catFlt==='all'?'rgba(31,215,96,.35)':'var(--b1)'}`, color:catFlt==='all'?'#1FD760':'var(--t2)' }}>
          <div style={{ fontSize:18, marginBottom:2 }}>🏪</div>
          <div style={{ fontSize:10, fontWeight:700 }}>Все</div>
          <div style={{ fontSize:10, opacity:.7 }}>{prods.length}</div>
        </button>
        {byCat.map(c=>(
          <button key={c.id} type="button" onClick={()=>setCatFlt(c.id)} className="ab" style={{ flexShrink:0, minWidth:72, padding:'10px 12px', borderRadius:12, textAlign:'center', background:catFlt===c.id?'rgba(31,215,96,.14)':'var(--l2)', border:`1.5px solid ${catFlt===c.id?'rgba(31,215,96,.35)':'var(--b1)'}`, color:catFlt===c.id?'#1FD760':'var(--t2)' }}>
                ...
                <tr key={p.id} onClick={()=>setEditP(p)} style={{ cursor:'pointer', transition:'background .15s' }} onMouseEnter={e=>{ e.currentTarget.style.background='rgba(31,215,96,.04)'; }} onMouseLeave={e=>{ e.currentTarget.style.background='transparent'; }}>
                  ...
                  <td>
                    ...
                    <div>
                        ...
                        {p.desc && <div style={{fontSize:10,color:'var(--t3)',marginTop:2,maxWidth:220,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{p.desc}</div>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td style={{ fontSize:10, color: bulkHint ? '#FF8C00' : 'var(--t3)', maxWidth:110, lineHeight:1.35 }}>
                    {bulkHint || '—'}
                  </td>
                  <td style={{fontSize:11,color:'var(--t2)'}}>{p.unit}{isWeighted(p)?' ⚖️':''}</td>
                  <td onClick={e=>e.stopPropagation()}>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <div style={{width:52,height:7,borderRadius:4,background:'var(--b1)',overflow:'hidden'}}><div style={{height:'100%',width:`${Math.min(100,p.stock/25*100)}%`,background:sc.c,borderRadius:4}}/></div>
                      <span style={{fontSize:12,fontWeight:800,color:sc.c,minWidth:18}}>{p.stock}</span>
                    </div>
                  </td>
                  <td onClick={e=>e.stopPropagation()}>
                    <div onClick={()=>void saveProduct({ ...p, hot: !p.hot }).catch(()=>{})} style={{width:36,height:20,borderRadius:10,background:p.hot?'#1FD760':'var(--b2)',position:'relative',cursor:'pointer',flexShrink:0}}>
                      <div style={{position:'absolute',top:2,left:p.hot?17:2,width:16,height:16,borderRadius:'50%',background:'white',transition:'left .2s'}}/>
                    </div>
                  </td>
                  <td onClick={e=>e.stopPropagation()}>
                    <div onClick={()=>void saveProduct({ ...p, organic: !p.organic }).catch(()=>{})} style={{width:36,height:20,borderRadius:10,background:p.organic?'#34D399':'var(--b2)',position:'relative',cursor:'pointer',flexShrink:0}}>
        ...
        <div className="amod">
          ...
          <div className="amodbox" style={{maxWidth:580}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
              ...
              <button onClick={closeAddModal} className="ab" style={{background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',width:32,height:32,padding:0,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:10,fontSize:16}}>✕</button>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'72px 1fr 1fr',gap:12}}>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Emoji</div><input className="ai" value={nEmoji} onChange={e=>setNEmoji(e.target.value)} style={{textAlign:'center',fontSize:24,height:48}}/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Название *</div><input className="ai" value={nName} onChange={e=>setNName(e.target.value)} placeholder="Название товара"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Артикул</div><input className="ai" value={nArt} onChange={e=>setNArt(e.target.value.replace(/\D/g,''))} placeholder="Авто (свободный номер)"/></div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Цена (ЅМ) *</div><input className="ai" value={nPrice} onChange={e=>setNPrice(e.target.value)} type="number" placeholder="0.00"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Цена прихода (ЅМ)</div><input className="ai" value={nCostPrice} onChange={e=>setNCostPrice(e.target.value)} type="number" step="0.01" placeholder="Закупка"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Остаток</div><input className="ai" value={nStock} onChange={e=>setNStock(e.target.value)} type="number" placeholder="0"/></div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                ...
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Единица измерения</div><input className="ai" value={nUnit} onChange={e=>setNUnit(e.target.value)} placeholder="500 гр / 1 кг / 1 л ..."/></div>
              </div>
              <div>
              ...
              <div style={{display:'flex',gap:8}}>
                {[{id:'piece',l:'📦 Поштучно'},{id:'weight',l:'⚖️ На развес'}].map(t=>(
                  <button key={t.id} type="button" onClick={()=>setNSellType(t.id)} className="ab" style={{flex:1,padding:'8px',fontSize:12,background:nSellType===t.id?'rgba(31,215,96,.14)':'var(--l2)',border:`1.5px solid ${nSellType===t.id?'rgba(31,215,96,.35)':'var(--b1)'}`,color:nSellType===t.id?'#1FD760':'var(--t2)'}}>{t.l}</button>
                ))}
              </div>
              {nSellType==='weight'&&(
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                  <div><div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>Цена за (г)</div><input className="ai" value={nUnitGrams} onChange={e=>setNUnitGrams(e.target.value)} type="number"/></div>
                  <div><div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>Шаг в корзине (г)</div><input className="ai" value={nWeightStep} onChange={e=>setNWeightStep(e.target.value)} type="number"/></div>
                </div>
              )}
              <BulkPricingFields tiers={nBulkPricing} onChange={setNBulkPricing} sellType={nSellType} />
              <div style={{display:'flex',gap:16,padding:'10px 14px',borderRadius:11,background:'var(--l3)',border:'1px solid var(--b1)'}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:'var(--t2)'}}>{editForm.art}</div>
                </div>
              </div>
              <button onClick={()=>setEditP(null)} className="ab" style={{background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',width:32,height:32,padding:0,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:10,fontSize:16}}>✕</button>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              <PhotoUploadField
                value={ePhoto}
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Бренд</div><input className="ai" value={editForm.brand} onChange={e=>setEditForm(f=>({...f,brand:e.target.value}))} placeholder="Производитель"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Страна</div><input className="ai" value={editForm.country} onChange={e=>setEditForm(f=>({...f,country:e.target.value}))} placeholder="Таджикистан"/></div>
                <div>
                  <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Штрих-код</div>
              ...
              <div style={{padding:'12px 14px',borderRadius:12,background:'var(--l3)',border:'1px solid var(--b1)'}}>
                <div style={{fontSize:11,color:'var(--t2)',marginBottom:8,fontWeight:700}}>Тип продажи</div>
                <div style={{display:'flex',gap:8,marginBottom:10}}>
                  {[{id:'piece',l:'📦 Поштучно'},{id:'weight',l:'⚖️ На развес'}].map(t=>(
                    <button key={t.id} type="button" onClick={()=>setEditForm(f=>({...f,sellType:t.id}))} className="ab" style={{flex:1,padding:'9px 8px',fontSize:12,background:editForm.sellType===t.id?'rgba(31,215,96,.14)':'var(--l2)',border:`1.5px solid ${editForm.sellType===t.id?'rgba(31,215,96,.35)':'var(--b1)'}`,color:editForm.sellType===t.id?'#1FD760':'var(--t2)'}}>{t.l}</button>
                  ))}
                </div>
                {editForm.sellType==='weight' && (
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                    <div><div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>Цена за (г)</div><input className="ai" type="number" value={editForm.unitGrams} onChange={e=>setEditForm(f=>({...f,unitGrams:Number(e.target.value)}))}/></div>
                    <div><div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>Шаг (+г)</div><input className="ai" type="number" value={editForm.weightStep} onChange={e=>setEditForm(f=>({...f,weightStep:Number(e.target.value)}))}/></div>
                    <div><div style={{fontSize:10,color:'var(--t3)',marginBottom:4}}>Минимум (г)</div><input className="ai" type="number" value={editForm.minWeight} onChange={e=>setEditForm(f=>({...f,minWeight:Number(e.target.value)}))}/></div>
                  </div>
                )}
              ...
              />
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Цена продажи (ЅМ)</div><input className="ai" value={editForm.price} onChange={e=>setEditForm(f=>({...f,price:e.target.value}))} type="number" step="0.01"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Цена прихода (ЅМ)</div><input className="ai" value={editForm.costPrice} onChange={e=>setEditForm(f=>({...f,costPrice:e.target.value}))} type="number" step="0.01" placeholder="Закупка"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Остаток</div><input className="ai" value={editForm.stock} onChange={e=>setEditForm(f=>({...f,stock:e.target.value}))} type="number"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Единица (отображение)</div><input className="ai" value={editForm.unit} onChange={e=>setEditForm(f=>({...f,unit:e.target.value}))} placeholder="500 гр / шт / 1 л"/></div>
              </div>
              {editForm.costPrice !== '' && editForm.price && Number(editForm.price) > Number(editForm.costPrice) && (
                <div style={{fontSize:11,color:'var(--t2)',padding:'8px 12px',borderRadius:9,background:'rgba(31,215,96,.06)',border:'1px solid rgba(31,215,96,.15)'}}>
...
function PartnersPage() {
  ...
  const renderPayoutHistoryItem = (p: any) => (
    <div key={p.id} style={{padding:'10px 11px',borderRadius:10,background:'var(--l2)',border:'1px solid var(--b1)',marginBottom:8}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:6}}>
        <div>
          ...
          <div style={{fontSize:11,color:'var(--t2)'}}>{p.date}</div>
          <div style={{fontSize:10,color:'var(--t3)',marginTop:2}}>{payMethodLabel(p.method)}</div>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'4px 10px',fontSize:10,color:'var(--t3)'}}>
        <span>Выручка (часть): <span style={{color:'var(--t2)'}}>{(p.revenuePaid ?? p.revenue ?? 0).toLocaleString()} ЅМ</span></span>
        <span>Комиссия {p.commissionPct}%: <span style={{color:'#FF4545'}}>−{(p.commission ?? 0).toLocaleString()} ЅМ</span></span>
        {(p.revenueTotal ?? 0) > 0 && <span>Выручка периода: <span style={{color:'var(--t2)'}}>{p.revenueTotal.toLocaleString()} ЅМ</span></span>}
        {(p.paidNetBefore ?? 0) > 0 && <span>Было к выплате: <span style={{color:'var(--t2)'}}>{p.paidNetBefore.toLocaleString()} ЅМ</span></span>}
        {(p.orders ?? 0) > 0 && <span>Заказов: <span style={{color:'var(--t2)'}}>{p.orders}</span></span>}
      </div>
      {p.note && <div style={{fontSize:10,color:'var(--t2)',marginTop:6,paddingTop:6,borderTop:'1px solid var(--b1)'}}>💬 {p.note}</div>}
    </div>
  );
  ...
  return (
    <div>
            ...
            <div style={{padding:'12px 14px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                <div style={{display:'flex',gap:14}}>
                  <div style={{textAlign:'center'}}><div style={{fontSize:9,color:'var(--t3)'}}>Заказов</div><div className="ub" style={{fontSize:13,fontWeight:800}}>{r.ordersMonth}</div></div>
                  <div style={{textAlign:'center'}}><div style={{fontSize:9,color:'var(--t3)'}}>Выручка</div><div className="ub" style={{fontSize:13,fontWeight:800}}>{(r.revenueMonth/1000).toFixed(1)}k</div></div>
      ...
      {/* Комиссии */}
      <div className="ac">
        <div style={{padding:'12px 16px',borderBottom:'1px solid var(--b1)',fontWeight:800,fontSize:13}}>Комиссии за месяц</div>
        <table className="at">
          ...
          <tbody>
            {rests.map(r=>(
              <tr key={r.id}>
                ...
                <td><div style={{display:'flex',alignItems:'center',gap:6}}><input type="number" defaultValue={r.commission} className="ai" style={{width:60,padding:'4px 8px',fontSize:12,textAlign:'center'}}/><span style={{fontSize:11,color:'var(--t3)'}}>%</span></div></td>
        ...
        <div className="amod">
          ...
          <div className="amodbox" style={{maxWidth:700,maxHeight:'92vh',overflowY:'auto'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:28}}>{sel.emoji}</span><div><div className="ub" style={{fontSize:14,fontWeight:800}}>{sel.name}</div><div style={{fontSize:11,color:'var(--t2)'}}>{sel.cuisine} · {sel.address}</div></div></div>
              ...
              <div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
                  {[
                    ...
                    <div key={i}>
                      <div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>{f.l}</div>
                ...
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 14px',borderRadius:11,background:!editForm.blocked?'rgba(31,215,96,.07)':'rgba(255,69,69,.07)',border:`1px solid ${!editForm.blocked?'rgba(31,215,96,.2)':'rgba(255,69,69,.2)'}`,marginBottom:12}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:'var(--t2)',marginTop:2}}>{!editForm.blocked?'Ресторан виден клиентам':'Не принимает заказы'}</div>
                </div>
                ...
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 14px',borderRadius:11,background:editForm.open?'rgba(31,215,96,.07)':'rgba(255,69,69,.07)',border:`1px solid ${editForm.open?'rgba(31,215,96,.2)':'rgba(255,69,69,.2)'}`,marginBottom:14,opacity:editForm.blocked?.5:1}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:'var(--t2)',marginTop:2}}>{editForm.open?'Принимает заказы сейчас':'Заказы остановлены'}</div>
                  </div>
                ...
                <div style={{padding:'9px 12px',borderRadius:9,background:'rgba(59,142,240,.06)',border:'1px solid rgba(59,142,240,.2)',fontSize:11,color:'var(--t2)',marginBottom:12}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {sel.menu.map(item=>(
                    <div key={item.id} style={{display:'flex',alignItems:'center',gap:11,padding:'10px 13px',background:'var(--l3)',borderRadius:11,border:`1px solid ${item.inStock?'var(--b1)':'rgba(255,69,69,.3)'}`}}>
                      <div style={{width:38,height:38,borderRadius:9,background:'var(--b1)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{item.e}</div>
                ...
                {ALL_ORDERS.map((o,i)=>{
                  ...
                  return (
                    <div key={i} style={{padding:'10px 13px',background:'var(--l3)',borderRadius:11,border:`1px solid ${s.c}20`,marginBottom:8}}>
                      ...
                      <div style={{fontSize:11,color:'var(--t2)'}}>{o.client} · {o.items}</div>
                    </div>
                  );
                })}
              </div>
              ...
              {payDone && lastPaidPayout ? (
                <div style={{padding:'10px 4px'}}>
                  <div style={{textAlign:'center',marginBottom:16}}>
                    ...
                    <div style={{fontSize:13,color:'var(--t2)',marginBottom:6}}>{showPay.emoji} {showPay.name}</div>
                    <div className="ub" style={{fontSize:22,fontWeight:900,color:'#1FD760',marginBottom:8}}>
                      {(lastPaidPayout.amount ?? 0).toLocaleString()} ЅМ
                    </div>
                    <div style={{fontSize:12,color:'var(--t3)'}}>{payMethodLabel(lastPaidPayout.method || payMethod)} · {lastPaidPayout.date}</div>
                  </div>
                ...
                <>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                    <div style={{display:'flex',alignItems:'center',gap:10}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:11,color:'var(--t2)'}}>{showPay.name}</div>
                      </div>
                    </div>
                    <button onClick={closePay} className="ab" style={{background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',width:32,height:32,padding:0,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:10,fontSize:16}}>✕</button>
                  </div>
                  {[
                    {l:'Выручка за период',v:`${revenue.toLocaleString()} ЅМ`,c:'var(--t1)'},
                    ...
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'11px 14px',background:i===3||(paidGross===0&&i===2)?'rgba(31,215,96,.06)':'var(--l3)',borderRadius:10,border:`1px solid ${i===3||(paidGross===0&&i===2)?'rgba(31,215,96,.25)':'var(--b1)'}`,marginBottom:8}}>
                      <span style={{fontSize:12,color:'var(--t2)'}}>{row.l}</span>
                      <span className="ub" style={{fontSize:13,fontWeight:800,color:row.c}}>{row.v}</span>
                    </div>
                  ))}
                  {paidGross > 0 && (
                    <div style={{fontSize:10,color:'var(--t3)',marginBottom:10,padding:'0 2px'}}>
                  ...
                  <div style={{display:'flex',gap:8,marginBottom:10}}>
                    {(['full','partial'] as const).map(mode=>(
                      ...
                      }} className="ab" style={{flex:1,padding:'8px 10px',fontSize:11,background:payMode===mode?'rgba(31,215,96,.12)':'var(--l3)',border:`1.5px solid ${payMode===mode?'rgba(31,215,96,.35)':'var(--b1)'}`,color:payMode===mode?'#1FD760':'var(--t2)'}}>
                  ...
                  {payMode === 'partial' && net > 0 && (
                    <div style={{marginBottom:10}}>
                      <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Сумма к выплате (ЅМ)</div>
                      ...
                      <div style={{display:'flex',gap:6,marginTop:8,flexWrap:'wrap'}}>
                        {[0.25,0.5,0.75,1].map(frac=>(
                          <button key={frac} type="button" onClick={()=>setPayAmountInput(String(Math.round(net*frac)))} className="ab" style={{padding:'5px 10px',fontSize:10,background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)'}}>
                  ...
                  <div style={{marginBottom:10,marginTop:4}}>
                    <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Способ выплаты</div>
                  ...
                  <div style={{marginBottom:12}}>
                    <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Комментарий (необязательно)</div>
                  ...
                  {restHistory.length > 0 && (
                    <div style={{marginBottom:12,padding:'10px 12px',borderRadius:10,background:'var(--l3)',border:'1px solid var(--b1)'}}>
                      <div style={{fontSize:10,color:'var(--t3)',marginBottom:8,fontWeight:700}}>ИСТОРИЯ ВЫПЛАТ ({restHistory.length})</div>
      ...
      {showAdd&&(
        <div className="amod">
          ...
          <div className="amodbox" style={{maxWidth:560,maxHeight:'92vh',overflowY:'auto'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}><div className="ub" style={{fontSize:14,fontWeight:800}}>Добавить ресторан</div><button onClick={()=>{setShowAdd(false);resetAddForm();}} className="ab" style={{background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',width:32,height:32,padding:0,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:10,fontSize:16}}>✕</button></div>
            <div style={{display:'flex',flexDirection:'column',gap:11}}>
              <div style={{display:'grid',gridTemplateColumns:'66px 1fr',gap:10}}>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Emoji</div><input className="ai" value={addForm.emoji} onChange={e=>setAddForm(f=>({...f,emoji:e.target.value}))} style={{textAlign:'center',fontSize:22,height:46}}/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Название *</div><input className="ai" value={addForm.name} onChange={e=>setAddForm(f=>({...f,name:e.target.value}))} placeholder="Название ресторана"/></div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Кухня</div><input className="ai" value={addForm.cuisine} onChange={e=>setAddForm(f=>({...f,cuisine:e.target.value}))} placeholder="Таджикская..."/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Адрес</div><input className="ai" value={addForm.address} onChange={e=>setAddForm(f=>({...f,address:e.target.value}))} placeholder="ул. Ленина, 42"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Email партнёра *</div><input className="ai" value={addForm.email} onChange={e=>setAddForm(f=>({...f,email:e.target.value}))} placeholder="partner@rest.tj"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Пароль *</div><input className="ai" type="password" value={addForm.password} onChange={e=>setAddForm(f=>({...f,password:e.target.value}))} placeholder="••••••••"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Телефон</div><input className="ai" value={addForm.phone} onChange={e=>setAddForm(f=>({...f,phone:e.target.value}))} placeholder="+992 __ ___"/></div>
                <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>Комиссия %</div><input className="ai" type="number" value={addForm.commission} onChange={e=>setAddForm(f=>({...f,commission:e.target.value}))} placeholder="15"/></div>
              </div>
              <AdminLocationMap
                mapKey="rest-add"
                lat={addForm.lat}
              ...
              <div style={{padding:'9px 12px',borderRadius:9,background:'rgba(31,215,96,.05)',border:'1px solid rgba(31,215,96,.18)',fontSize:11,color:'var(--t2)'}}>
...
function ReviewsPage() {
  ...
  const Stars = ({ n }: { n: number }) => (
    <span>{[1, 2, 3, 4, 5].map(i => <span key={i} style={{ color: i <= n ? '#FFB800' : 'var(--b2)', fontSize: 13 }}>★</span>)}</span>
  );
    ...
    return (
      <button
        ...
        style={{
          ...
          border: `1.5px solid ${on ? `${color}55` : 'var(--b1)'}`,
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {!loading && visible.length === 0 && (
          <div style={{ textAlign: 'center', padding: 40, color: 'var(--t2)', fontSize: 13 }}>
        ...
        {visible.map((rev, i) => {
          ...
          return (
            <div key={rev.id} className="ac" style={{ padding: '15px 17px', border: `1.5px solid ${selected.has(rev.id) ? 'rgba(31,215,96,.45)' : rev.status === 'new' ? 'rgba(255,69,69,.3)' : 'var(--b1)'}`, animation: `fadeUp .4s ease ${i * .06}s both` }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      ...
                      {rev.orderId && <span style={{ fontSize: 10, color: 'var(--t3)' }}>· {rev.orderId}</span>}
                </div>
                </div>
              </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 8, background: 'rgba(0,0,0,.3)', border: '1px solid var(--b1)' }}>
                    <span style={{ fontSize: 11, color: 'var(--t2)' }}>{place}</span>
              </div>
              ...
              <div style={{ fontSize: 13, color: 'var(--t1)', lineHeight: 1.6, marginBottom: 10, padding: '9px 12px', background: 'var(--l3)', borderRadius: 9, border: '1px solid var(--b1)' }}>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 12 }}>
        ...
        <StatCard l="Офлайн" v={couriers.filter(c => c.status === 'offline' || c.blocked).length} c="var(--t3)" />
      </div>
        ...
        {([
          ...
          <button
            ...
            style={{
              ...
              alignItems: 'center',
              gap: 7,
              background: section === id ? 'rgba(31,215,96,.12)' : 'var(--l3)',
              border: `1.5px solid ${section === id ? 'rgba(31,215,96,.4)' : 'var(--b1)'}`,
              color: section === id ? '#1FD760' : 'var(--t2)',
            }}
          >
            {icon} {label}
      ...
      {section === 'wallet' && (
        <>
          <div className="ac" style={{ padding: '14px 16px', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ fontSize: 10, color: 'var(--t2)', marginTop: 3 }}>
                  % от доставки списывается при принятии заказа · на счетах: <b style={{ color: 'var(--t1)' }}>{formatSm(walletSummary.totalBalance)}</b>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                />
                <span style={{ fontSize: 12, color: 'var(--t3)', fontWeight: 700 }}>%</span>
                {[5, 10, 15, 20].map(n => (
            ...
            <div style={{ marginTop: 10, fontSize: 10, color: 'var(--t2)' }}>
            ...
            <div className="ac" style={{ marginBottom: 18 }}>
              ...
              <div style={{ maxHeight: 520, overflowY: 'auto' }}>
                {filteredWalletCouriers.length === 0 ? (
                  <div style={{ padding: 32, textAlign: 'center', color: 'var(--t3)', fontSize: 12 }}>Курьеры не найдены</div>
                ) : (
                  <table className="at">
                    <thead>
                      <tr>
                        ...
                        return (
                          ...
                          >
                            <td>
                              ...
                              <div style={{ fontSize: 10, color: 'var(--t2)' }}>{c.phone}</div>
                            </td>
                            <td>
                            ...
                            <td>
                              <span className="ub" style={{ fontSize: 12, fontWeight: 800, color: low ? '#FF4545' : 'var(--t1)' }}>{formatSm(balance)}</span>
                            </td>
                            <td style={{ fontSize: 11, color: 'var(--t2)' }}>
                              {formatCourierCommissionPercent(getCourierCommissionPercent(tariff, c))}
                              <div style={{ fontSize: 9, color: 'var(--t3)' }}>~{formatSm(commission)}</div>
                            </td>
            ...
            <div style={{ marginBottom: 18 }}>
              <div className="ac" style={{ padding: '12px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontSize: 9, color: 'var(--t2)' }}>Баланс</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.25fr', gap: 14 }}>
                <div className="ac" style={{ padding: 16 }}>
                    ...
                    {([
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: walletActionMode === id ? (id === 'withdraw' ? 'rgba(255,69,69,.12)' : 'rgba(31,215,96,.12)') : 'var(--l3)',
                          border: `1px solid ${walletActionMode === id ? (id === 'withdraw' ? 'rgba(255,69,69,.35)' : 'rgba(31,215,96,.35)') : 'var(--b1)'}`,
                          color: walletActionMode === id ? (id === 'withdraw' ? '#FF6969' : '#1FD760') : 'var(--t2)',
                        }}
                      >
                        {label}
                      </button>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--t2)', marginBottom: 12, lineHeight: 1.45 }}>
                    ...
                    {' · '}баланс <b style={{ color: 'var(--t1)' }}>{formatSm(getCourierBalance(selectedWalletCourier))}</b>
                  </div>
                ...
                <div className="ac" style={{ padding: 16, display: 'flex', flexDirection: 'column' }}>
                    ...
                    {([
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: walletTxFilter === id ? 'rgba(31,215,96,.12)' : 'var(--l3)',
                          border: `1px solid ${walletTxFilter === id ? 'rgba(31,215,96,.35)' : 'var(--b1)'}`,
                          color: walletTxFilter === id ? '#1FD760' : 'var(--t2)',
                        }}
                      >
                        {label}
                      </button>
                    ))}
                    <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--t2)', alignSelf: 'center' }}>
                  ...
                  <div style={{ flex: 1, maxHeight: 360, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {depositWalletLoading && depositWalletTx.length === 0 ? (
                      <div style={{ padding: 24, textAlign: 'center', color: 'var(--t2)', fontSize: 12 }}>Загрузка…</div>
                    ) : depositWalletTx.filter(t => walletTxFilter === 'all' || t.type === walletTxFilter).length === 0 ? (
                      <div style={{ padding: 24, textAlign: 'center', color: 'var(--t3)', fontSize: 12, background: 'var(--l2)', borderRadius: 10, border: '1px solid var(--b1)' }}>
                        Операций пока нет
                      </div>
                    ) : depositWalletTx
                      ...
                      .map(tx => {
                        ...
                        return (
                          <div key={tx.id} style={{ padding: '9px 10px', borderRadius: 10, background: 'var(--l2)', border: `1px solid ${positive ? 'rgba(31,215,96,.15)' : 'rgba(255,69,69,.15)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                              <span style={{ fontSize: 14 }}>{icon}</span>
                              <div style={{ minWidth: 0 }}>
                                <div style={{ fontSize: 11, fontWeight: 800 }}>{walletTxLabel(tx.type)}</div>
                                <div style={{ fontSize: 9, color: 'var(--t2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tx.note || '—'}</div>
                              </div>
                            </div>
                            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                              <div className="ub" style={{ fontSize: 12, fontWeight: 800, color }}>{positive ? '+' : ''}{formatSm(tx.amount)}</div>
                              <div style={{ fontSize: 9, color: 'var(--t3)' }}>{formatWalletTxTime(tx.at)}</div>
                              {tx.balanceAfter != null && (
                                <div style={{ fontSize: 8, color: 'var(--t3)', marginTop: 2 }}>баланс {formatSm(tx.balanceAfter)}</div>
                              )}
                            </div>
                          </div>
                        );
                ...
                <tr key={c.id} style={c.blocked ? { opacity: .65 } : undefined}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 13 }}>{c.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--t2)' }}>{c.phone}</div>
            </div>
        </div>
                  </td>
                  <td style={{ fontSize: 12, color: 'var(--t2)' }}>
                    {vehicleIcon(c.vehicle)} {vehicleLabel(c.vehicle)}
                    <div style={{ fontSize: 10, color: 'var(--t3)' }}>{c.num}</div>
                  </td>
                  ...
                  <td>
                    <span className="ub" style={{ fontSize: 12, fontWeight: 800, color: lowBalance ? '#FF4545' : 'var(--t1)' }}>
                      {formatSm(balance)}
                    </span>
      ...
      <div className="ac" style={{ marginTop: 18 }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--b1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 4 }}>Только сумма доставки · товары не включены</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
          ...
          <tbody>
            {financeRows.map(r => (
              <tr key={r.id}>
                ...
                <td style={{ fontSize: 12, color: 'var(--t2)' }}>{vehicleLabel(r.vehicle as AdminCourier['vehicle'])}</td>
                <td>{r.deliveries}</td>
                <td><span className="ub" style={{ color: '#3B8EF0', fontWeight: 800 }}>{formatSm(r.earnings)}</span></td>
                <td style={{ color: 'var(--t2)' }}>{r.deliveries ? formatSm(r.earnings / r.deliveries) : '—'}</td>
                <td style={{ color: '#FFB800' }}>★ {r.rating}</td>
              </tr>
            ))}
            {!financeRows.length && (
              <tr>
                <td colSpan={6} style={{ textAlign: 'center', padding: 24, color: 'var(--t3)' }}>Нет данных по курьерам</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="ac" style={{ marginTop: 14 }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--b1)', fontWeight: 800, fontSize: 13 }}>
        ...
        <table className="at">
          ...
          <tbody>
            {deliveryOrders.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ textAlign: 'center', padding: 24, color: 'var(--t3)' }}>Пока нет доставленных заказов</td>
              </tr>
            ) : deliveryOrders.map(o => (
              <tr key={o.id}>
                <td><span className="ub" style={{ fontSize: 11, color: '#1FD760' }}>{o.id}</span></td>
                <td style={{ fontSize: 12, fontWeight: 600 }}>{o.courier}</td>
                <td style={{ fontSize: 12, color: 'var(--t2)' }}>{o.client}</td>
                <td style={{ color: 'var(--t2)' }}>{formatSm(o.goodsTotal)}</td>
                <td><span className="ub" style={{ fontSize: 12, fontWeight: 800, color: '#3B8EF0' }}>{formatSm(o.deliveryFee)}</span></td>
                <td style={{ fontSize: 11, color: 'var(--t2)' }}>{o.km ? formatKm(o.km) : '—'}</td>
                <td style={{ fontSize: 11, color: 'var(--t3)' }}>{o.time || '—'}</td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 420 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={closeModal} className="ab" style={{ background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)', width: 32, height: 32, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 10, fontSize: 16 }}>✕</button>
              </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 4, fontWeight: 700 }}>Транспорт</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 4, fontWeight: 700 }}>Статус</div>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 18 }}>
        ...
        <StatCard l="Офлайн" v={assemblers.filter(a => a.status === 'offline' || a.blocked).length} c="var(--t3)" />
      </div>
            ...
            {assemblers.map(a => {
              ...
              return (
                <tr key={a.id} style={a.blocked ? { opacity: .65 } : undefined}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--t2)' }}>{a.phone}</div>
                      </div>
                    </div>
                  </td>
                  <td><Badge v={s.l} c={s.c} /></td>
                  <td>
                    <span className="ub" style={{ fontSize: 12, fontWeight: 800, color: active ? '#9B6DFF' : 'var(--t3)' }}>
                      {active}
                    </span>
                  </td>
                  ...
                  <td style={{ color: 'var(--t2)' }}>{a.ordersTotal}</td>
                  <td style={{ color: 'var(--t2)' }}>{formatAssemblerAvgTime(a.avgTimeMin)}</td>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12, alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 220px', maxWidth: 360 }}>
          ...
          {search && (
            <button
              onClick={() => setSearch('')}
              style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--t2)', cursor: 'pointer', fontSize: 16 }}
            >✕</button>
          )}
        </div>
        {search.trim() && (
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: '5px 12px',
              fontSize: 11,
              background: filterAccount === f.id ? (f.id === 'recovery' ? 'rgba(255,140,0,.15)' : 'rgba(31,215,96,.15)') : 'var(--l3)',
              color: filterAccount === f.id ? (f.id === 'recovery' ? '#FF8C00' : '#1FD760') : 'var(--t2)',
              border: `1px solid ${filterAccount === f.id ? (f.id === 'recovery' ? 'rgba(255,140,0,.35)' : 'rgba(31,215,96,.35)') : 'var(--b1)'}`,
            }}
          >{f.l}</button>
        ))}
        <select
      ...
      {selectedIds.size > 0 && (
        <div style={{
          ...
          <button
            ...
            disabled={bulkDeleting}
            className="ab"
            style={{ padding: '6px 14px', fontSize: 12, background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)' }}
          >
            Снять выбор
          </button>
        </div>
        ...
        <table className="at">
          ...
          <tbody>
            {USE_API && clientsApiSyncing && !stored.length ? (
              <tr>
                <td colSpan={12} style={{ textAlign: 'center', color: 'var(--t2)', padding: 28 }}>
              ...
              <tr>
                <td colSpan={12} style={{ textAlign: 'center', color: 'var(--t3)', padding: 28 }}>
              ...
              return (
                <tr key={c.id} style={(c.blocked || isClientInRecovery(c)) ? { opacity: .75 } : undefined}>
                  ...
                  <td><span style={{ fontSize: 11, color: 'var(--t2)' }}>{clientSegmentLabel(seg)}</span></td>
                  <td>{c.card ? <span className="ub" style={{ fontSize: 11, fontWeight: 800, color: '#FFB800' }}>{c.card}</span> : <span style={{ color: 'var(--t3)' }}>—</span>}</td>
                  <td><Badge v={c.level} c={LVC[c.level] || 'var(--t2)'} /></td>
                  ...
                  <td style={{ color: c.debt > 0 ? '#FF4545' : 'var(--t3)', fontWeight: c.debt > 0 ? 800 : 400 }}>{c.debt > 0 ? `${c.debt.toLocaleString()} ЅМ` : '—'}</td>
                  <td style={{ color: '#FFB800', fontWeight: 600 }}>{c.bonus.toLocaleString()} ⭐</td>
                  <td style={{ fontSize: 11, color: 'var(--t3)' }}>{c.lastLabel}</td>
                  <td>
                    <div style={{ display: 'flex', gap: 5 }}>
                      <a href={`tel:${c.phone.replace(/\s/g, '')}`} className="ab abg" style={{ padding: '4px 9px', fontSize: 11, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>📱</a>
      ...
      {showAdd && (
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 480 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
              <div style={{ fontSize: 10, color: 'var(--t3)', fontWeight: 700, letterSpacing: .3 }}>ПРОФИЛЬ КЛИЕНТА</div>
              <NI lbl="ФИО *" val={form.name} set={v => setF('name', v)} ph="Имя Фамилия" />
              <NI lbl="Телефон *" val={form.phone} set={v => setF('phone', v)} ph="+992 __ ___ __ __" />
              ...
              <div>
                <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 4, fontWeight: 700 }}>Карта КАКАПО</div>
                ...
                <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 4 }}>Привязка карты · уровень и бонусы — в разделе «Карты»</div>
              </div>
              {formLoyalty?.card && (
                <div style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(255,184,0,.06)', border: '1px solid rgba(255,184,0,.2)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
                    <div><span style={{ color: 'var(--t3)' }}>Уровень:</span> <strong>{formLoyalty.level || '—'}</strong></div>
                    <div><span style={{ color: 'var(--t3)' }}>Бонусы:</span> <strong style={{ color: '#FFB800' }}>{formLoyalty.bonus.toLocaleString()} ⭐</strong></div>
                    <div><span style={{ color: 'var(--t3)' }}>Лимит:</span> <strong>{formLoyalty.debtLimit > 0 ? `${formLoyalty.debtLimit} ЅМ` : 'Нет'}</strong></div>
                    <div><span style={{ color: 'var(--t3)' }}>Долг:</span> <strong style={{ color: formLoyalty.debt > 0 ? '#FF4545' : 'var(--t2)' }}>{formLoyalty.debt > 0 ? `${formLoyalty.debt} ЅМ` : '—'}</strong></div>
                  </div>
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', borderRadius: 10, background: 'var(--l3)', border: '1px solid var(--b1)' }}>
                <div style={{ fontSize: 12, fontWeight: 700 }}>Заблокирован</div>
                <Tog on={form.blocked} set={() => setF('blocked', !form.blocked)} />
              </div>
              <div>
                <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 4, fontWeight: 700 }}>Заметка администратора</div>
      ...
      {deleteConfirm && (
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 420 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--t2)', lineHeight: 1.55, marginBottom: 16 }}>
              <strong style={{ color: 'var(--t1)' }}>{deleteConfirm.name}</strong> ({deleteConfirm.phone}) будет полностью удалён из базы.
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                className="ab"
                style={{ flex: 1, background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)' }}
              >
                Отмена
              </button>
            </div>
      ...
      {bulkDeleteOpen && (
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 460 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--t2)', lineHeight: 1.55, marginBottom: 12 }}>
            ...
            <div style={{
              ...
              {selectedClients.slice(0, 12).map(c => (
                <div key={c.id} style={{ padding: '3px 0' }}>
                  <strong style={{ color: 'var(--t1)' }}>{c.name}</strong> · {c.phone}
                </div>
              ))}
              {selectedClients.length > 12 && (
                <div style={{ color: 'var(--t3)', marginTop: 4 }}>…и ещё {selectedClients.length - 12}</div>
              )}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
      ...
      {recoveryConfirm && (
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 420 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--t2)', lineHeight: 1.55, marginBottom: 16 }}>
              <strong style={{ color: 'var(--t1)' }}>{recoveryConfirm.name}</strong> ({recoveryConfirm.phone}) попадёт в раздел «Восстановление».
        ...
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 520 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={() => setDetailId(null)} className="ab" style={{ background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)', width: 32, height: 32, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 10, fontSize: 16 }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
              {[
                ...
                <div key={row.l} style={{ padding: '8px 10px', borderRadius: 10, background: 'var(--l3)', border: '1px solid var(--b1)' }}>
                  <div style={{ fontSize: 10, color: 'var(--t3)', fontWeight: 700, marginBottom: 3 }}>{row.l}</div>
            ...
            {detailClient.note && (
              <div style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(255,184,0,.06)', border: '1px solid rgba(255,184,0,.2)', fontSize: 12, color: 'var(--t2)', marginBottom: 12 }}>
...
function CardVisualMini({ num, level, clientName, status }: { num?: string; level?: string; clientName?: string; status?: CardStatus }) {
  ...
  return (
    <div style={{
      ...
      {clientName ? (
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--t1)', marginBottom: 8 }}>{clientName}</div>
      ) : status === 'unlinked' ? (
        ...
        return (
          <button
            ...
            style={{
              ...
              fontWeight: 800,
              color: c,
              background: value === o.id ? `${c}28` : 'var(--l3)',
              border: `2px solid ${value === o.id ? c : 'var(--b2)'}`,
              borderRadius: 12,
              display: 'flex',
...
function CardFormSection({ title, subtitle, children }: { title: string; subtitle?: string; children: any }) {
  return (
    <div style={{ background: 'var(--l2)', border: '1px solid var(--b1)', borderRadius: 14, padding: '14px 16px' }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: '#1FD760', marginBottom: subtitle ? 4 : 10 }}>{title}</div>
      {subtitle && <div style={{ fontSize: 11, color: 'var(--t3)', marginBottom: 12, lineHeight: 1.45 }}>{subtitle}</div>}
      {children}
    </div>
  );
}
...
function ClientSearchPicker({
  ...
  return (
    <div>
      <div style={{ position: 'relative', marginBottom: 8 }}>
        ...
        {q && (
          <button
            type="button"
            onClick={() => setQ('')}
            style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--t2)', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}
          >
            ✕
          </button>
        )}
      ...
      <div style={{ fontSize: 10, color: 'var(--t3)', marginBottom: 8, fontWeight: 700 }}>
        {qTrim
        ...
        <div style={{ padding: '11px 13px', borderRadius: 12, background: 'rgba(31,215,96,.08)', border: '1px solid rgba(31,215,96,.25)', marginBottom: showList ? 10 : 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: 'var(--t2)', fontSize: 12, marginTop: 3 }}>{selected.phone}</div>
              {selected.card && (
            ...
            <button
              ...
              className="ab"
              style={{ flexShrink: 0, padding: '5px 10px', fontSize: 10, background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)' }}
            >
              Сменить
            </button>
          </div>
      ...
      {showList && (
        <div style={{
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: active ? '#1FD760' : 'var(--t1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {c.name}
                    </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
                    {c.card
                      ...
                      : <span style={{ fontSize: 10, color: 'var(--t3)' }}>без карты</span>}
          ...
          {hiddenCount > 0 && (
            <div style={{ padding: '8px 13px', fontSize: 10, color: 'var(--t3)', textAlign: 'center', borderTop: '1px solid var(--b1)' }}>
  ...
  return (
    <div>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              textAlign: 'left',
              borderRadius: 14,
              border: cardsTab === t.id ? '2px solid #1FD76055' : '1px solid var(--b1)',
              background: cardsTab === t.id ? 'rgba(31,215,96,.08)' : 'var(--l3)',
            }}
          >
            <div className="ub" style={{ fontSize: 14, fontWeight: 900, color: cardsTab === t.id ? '#1FD760' : 'var(--t1)', marginBottom: 4 }}>{t.l}</div>
            <div style={{ fontSize: 11, color: 'var(--t2)' }}>{t.d}</div>
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        {[
          { id: 'all', l: 'Всего карт', v: stats.total, c: 'var(--t1)', e: '💳' },
          ...
          <button
            ...
            style={{
              ...
              textAlign: 'left',
              cursor: 'pointer',
              border: filter === s.id ? `2px solid ${s.c}55` : '1px solid var(--b1)',
              background: filter === s.id ? `${s.c}0D` : undefined,
              transition: 'all .15s',
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 11, color: 'var(--t2)', fontWeight: 600 }}>{s.l}</span>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--t1)', marginBottom: 4 }}>Карта создаётся автоматически</div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 14, alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: 360 }}>
          ...
          />
          {search && (
            <button type="button" onClick={() => setSearch('')} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--t2)', cursor: 'pointer', fontSize: 16 }}>✕</button>
          )}
        </div>
        {search.trim() && (
          <span style={{ fontSize: 12, color: 'var(--t2)' }}>
      ...
      <div className="ac">
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--b1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--t3)' }}>{filtered.length} шт.</span>
        </div>
        <table className="at">
          <thead>
            <tr>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', padding: 32 }}>
                  ...
                  <div style={{ color: 'var(--t2)', fontWeight: 700, marginBottom: 4 }}>
              ...
              return (
                ...
                >
                  <td>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 2 }}>{c.phone || '—'}</div>
                  </td>
                      ...
                      ? (
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                            <Badge v={CARD_LEVEL_RU[c.level as ClientLevel] || c.level} c={LVC[c.level] || 'var(--t2)'} />
                            {c.vip && <Badge v="VIP" c="#FFB800" />}
                          </div>
                          <div style={{ fontSize: 10, color: 'var(--t2)', marginTop: 4, lineHeight: 1.4 }}>
        ...
        <div className="amod">
          ...
          <div className="amodbox" style={{ maxWidth: 480 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button type="button" onClick={() => setDetail(null)} className="ab" style={{ background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)', width: 32, height: 32, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 10, fontSize: 16 }}>✕</button>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, margin: '16px 0' }}>
              {[
                ...
                { l: 'Телефон', v: detail.phone || '—', c: 'var(--t2)' },
                ...
                { l: 'Долг', v: detail.debt > 0 ? `${detail.debt} ЅМ` : '—', c: detail.debt > 0 ? '#FF4545' : 'var(--t3)' },
                { l: 'VIP', v: detail.vip ? '👑 Включён' : 'Выключен', c: detail.vip ? '#FFB800' : 'var(--t3)' },
                { l: 'Срок уровня', v: formatAdminLevelExpiry({ level: detail.level as ClientLevel, levelAssignMode: detail.levelAssignMode, levelLockedPeriod: detail.levelLockedPeriod, levelValidUntil: detail.levelValidUntil }), c: 'var(--t2)' },
                { l: 'Срок VIP', v: detail.vip ? formatAdminVipExpiry({ vip: detail.vip, vipUntil: detail.vipUntil }) : '—', c: 'var(--t2)' },
                { l: 'Выдана', v: detail.issued || '—', c: 'var(--t2)' },
              ].map(row => (
                <div key={row.l} style={{ background: 'var(--l3)', borderRadius: 12, padding: '11px 13px', border: '1px solid var(--b1)' }}>
                  <div style={{ fontSize: 10, color: 'var(--t3)', fontWeight: 700, marginBottom: 4 }}>{row.l}</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: row.c }}>{row.v}</div>
                </div>
              ))}
            </div>
            {clientNoteForCard(detail, clients) && (
              <div style={{ padding: '10px 14px', borderRadius: 12, background: 'rgba(59,142,240,.08)', border: '1px solid rgba(59,142,240,.2)', fontSize: 12, color: 'var(--t2)', marginBottom: 14 }}>
                💬 {clientNoteForCard(detail, clients)}
        </div>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div className="amodbox" style={{ maxWidth: 460 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 3 }}>{showLink.num}</div>
              </div>
              <button type="button" onClick={() => setShowLink(null)} className="ab" style={{ background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)', width: 32, height: 32, padding: 0, borderRadius: 10, fontSize: 16 }}>✕</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 16 }}>
              ...
              <CardFormSection title="⭐ Бонусы" subtitle="Уровень, VIP и долг — во вкладке «Статусы»">
                ...
                <div style={{ marginTop: 12, padding: '10px 12px', borderRadius: 10, background: 'rgba(59,142,240,.08)', border: '1px solid rgba(59,142,240,.2)', fontSize: 11, color: 'var(--t2)', lineHeight: 1.45 }}>
...
function DebtsPage({ setPage }: { setPage: (p: string) => void }) {
  ...
  const applyDebtDelta = async (delta: number) => {
    ...
    const current = Math.max(0, Number(detail.debt) || 0);
    const nextDebt = Math.max(0, Math.round((current + delta) * 100) / 100);
    const form = {
  ...
  const deleteManualHistory = async (row: DebtHistoryEntry) => {
    ...
    setSaveErr('');
    try {
      await applyDebtDelta(debtBalanceDeltaForHistoryChange(removed, null));
      setHistTick(t => t + 1);
    } catch (e) {
      setSaveErr(e instanceof Error ? e.message : 'Не удалось обновить баланс');
    } finally {
      setSaveBusy(false);
    }
  };

  const saveManualHistoryEdit = async () => {
    ...
    const amountAbs = Math.max(0, parseFloat(histEdit.amount) || 0);
    if (!(amountAbs > 0)) {
      setSaveErr('Укажите сумму больше 0');
      return;
    }
    setHistEdit(prev => prev ? { ...prev, saving: true } : prev);
    setSaveErr('');
    try {
      const after = updateDebtHistoryEntry(detail.phone, histEdit.id, {
        amountAbs,
        desc: histEdit.desc,
      });
      if (!after) throw new Error('Не удалось сохранить');
      await applyDebtDelta(debtBalanceDeltaForHistoryChange(before, after));
      setHistEdit(null);
      setHistTick(t => t + 1);
    } catch (e) {
  ...
  return (
    <div>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              border: filter === s.filter && (i === 0 || i === 2 || i === 3) ? `2px solid ${s.c}55` : '1px solid var(--b1)',
      ...
      <div style={{
        ...
        💬 Клиенты погашают долг через <strong style={{ color: 'var(--t1)' }}>поддержку</strong> (звонок / Telegram).
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              background: filter === f.id ? 'rgba(255,140,0,.12)' : 'var(--l3)',
              border: `1.5px solid ${filter === f.id ? 'rgba(255,140,0,.35)' : 'var(--b1)'}`,
              color: filter === f.id ? '#FF8C00' : 'var(--t2)',
            }}
          >
            {f.l}
          </button>
      ...
      <div className="ac">
        <table className="at">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ textAlign: 'center', padding: '28px 14px', color: 'var(--t2)', fontSize: 13 }}>
              ...
              return (
                <tr key={c.num} onClick={() => openDetail(c)} style={{ cursor: 'pointer' }}>
                  ...
                  <td>
                    <span className="ub" style={{ fontSize: 13, fontWeight: 900, color: c.debt > 0 ? '#FF4545' : 'var(--t3)' }}>
                      {c.debt > 0 ? `${c.debt.toLocaleString()} ЅМ` : '—'}
                    </span>
                  </td>
                  <td style={{ fontSize: 12, color: 'var(--t2)' }}>{c.debtLimit > 0 ? `${c.debtLimit.toLocaleString()} ЅМ` : '—'}</td>
                  <td style={{ fontSize: 12, color: available > 0 ? '#1FD760' : 'var(--t3)', fontWeight: 700 }}>
          ...
          <div className="amodbox" style={{ maxWidth: 480 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 2 }}>{detail.phone || '—'}</div>
              </div>
              <button type="button" onClick={() => setDetail(null)} className="ab" style={{ background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)', width: 32, height: 32, padding: 0, borderRadius: 10, fontSize: 16 }}>✕</button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
              {[
                ...
                <div key={i} style={{ padding: '12px', borderRadius: 12, background: 'var(--l3)', border: '1px solid var(--b1)', textAlign: 'center' }}>
                  <div style={{ fontSize: 10, color: 'var(--t3)', marginBottom: 4, fontWeight: 700 }}>{row.l}</div>
            ...
            {creditOrders.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 160, overflowY: 'auto' }}>
                  {creditOrders.map(o => (
                    <div key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: 'var(--l3)', borderRadius: 10, border: '1px solid var(--b1)' }}>
                      <div>
                        <div className="ub" style={{ fontSize: 11, fontWeight: 800, color: '#1FD760' }}>{o.id}</div>
                        <div style={{ fontSize: 10, color: 'var(--t3)' }}>{o.createdAt || '—'}</div>
                      </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 280, overflowY: 'auto' }}>
                {debtHistory.length === 0 ? (
                  ...
                  return (
                    <div key={row.id} style={{ padding: '10px 12px', background: 'var(--l3)', borderRadius: 10, border: '1px solid var(--b1)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--t1)' }}>{desc || (isPay ? 'Погашение' : 'Начисление')}</span>
                            <span style={{ fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 6, background: 'var(--l2, rgba(0,0,0,.15))', color: 'var(--t3)' }}>{badge}</span>
                          </div>
                          {row.itemsSummary && <div style={{ fontSize: 10, color: 'var(--t2)', marginTop: 2 }}>{row.itemsSummary}</div>}
                          <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 3 }}>{row.date} · {row.time || '—'}</div>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          ...
                          {manual && !editing && (
                            <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end', marginTop: 6 }}>
                              <button
                                ...
                                style={{ fontSize: 10, padding: '4px 8px', background: 'var(--l2)', border: '1px solid var(--b1)', color: 'var(--t2)' }}
                              >
                                Изменить
                              </button>
                              <button
                                ...
                                style={{ fontSize: 10, padding: '4px 8px', background: 'rgba(255,69,69,.1)', border: '1px solid rgba(255,69,69,.3)', color: '#FF8080' }}
                              >
                                Удалить
                              </button>
                            </div>
                      ...
                      {editing && histEdit && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--b1)', display: 'grid', gap: 8 }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 8 }}>
                            <div>
                              <div style={{ fontSize: 10, color: 'var(--t3)', marginBottom: 4 }}>Сумма</div>
                              <input
                                className="ai"
                                type="number"
                                value={histEdit.amount}
                                disabled={histEdit.saving || saveBusy}
                                onChange={e => setHistEdit(prev => prev ? { ...prev, amount: e.target.value } : prev)}
                                style={{ width: '100%' }}
                              />
                            </div>
                            <div>
                              <div style={{ fontSize: 10, color: 'var(--t3)', marginBottom: 4 }}>Описание</div>
                              <input
                                className="ai"
                                type="text"
                                value={histEdit.desc}
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button
                              ...
                              style={{ fontSize: 11, padding: '6px 10px', fontWeight: 800 }}
                            >
                            ...
                            <button
                              ...
                              style={{ fontSize: 11, padding: '6px 10px', background: 'var(--l2)', border: '1px solid var(--b1)', color: 'var(--t2)' }}
                            >
                              Отмена
                            </button>
                          </div>
...
function PromosPage() {
    ...
    <div className="amod">
      ...
      <div className="amodbox" style={{maxWidth:480}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
          ...
          <button onClick={closeProductModal} className="ab" style={{background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',width:32,height:32,padding:0,display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div>
            <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Товар *</div>
            <ProductSearchPicker
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div>
              <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Цена по акции (ЅМ) *</div>
            ...
            <div>
              <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>Старая цена (ЅМ)</div>
          ...
          {productPreviewDisc > 0 && (
            <div style={{padding:'8px 12px',borderRadius:9,background:'rgba(255,69,69,.07)',border:'1px solid rgba(255,69,69,.2)',fontSize:12,color:'var(--t2)'}}>
              Скидка: <span style={{color:'#FF4545',fontWeight:800}}>−{productPreviewDisc}%</span>
            </div>
          )}
          <div>
            <div style={{fontSize:11,color:'var(--t2)',marginBottom:5,fontWeight:700}}>
            ...
            />
            <div style={{fontSize:10,color:'var(--t3)',marginTop:5,lineHeight:1.45}}>
            ...
            {editingProductPromo && Number(editingProductPromo.stockLimit) > 0 && (
              <div style={{marginTop:8,padding:'8px 10px',borderRadius:9,background:'rgba(59,142,240,.06)',border:'1px solid rgba(59,142,240,.15)',fontSize:11,color:'var(--t2)'}}>
                Продано: <strong style={{color:'var(--t1)'}}>{formatPromoStockAdmin(editingProductPromo, selectedProduct)}</strong>
  ...
  const renderPromoSelectionBar = (items: Promo[]) => {
    ...
    return (
      <div style={{
        ...
        padding: '10px 14px',
        borderRadius: 11,
        background: selectedInList.length ? 'rgba(255,69,69,.06)' : 'var(--l2)',
        border: `1px solid ${selectedInList.length ? 'rgba(255,69,69,.2)' : 'var(--b1)'}`,
        flexWrap: 'wrap',
      }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--t2)', cursor: 'pointer' }}>
        ...
        {selectedInList.length > 0 && (
          <>
            <span style={{ fontSize: 12, color: 'var(--t2)' }}>
              Выбрано: <strong style={{ color: 'var(--t1)' }}>{selectedInList.length}</strong>
            </span>
            <button
              type="button"
    ...
    return (
      <div key={p.id} className="ac" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ width: 44, height: 44, borderRadius: 13, background: 'var(--b1)', border: '1px solid #1E3522', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{prod?.e || p.e}</div>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 10, color: 'var(--t3)' }}>{prod?.art}</span>
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, color: p.on ? '#1FD760' : 'var(--t3)', fontWeight: 700 }}>{p.on ? 'Вкл' : 'Выкл'}</span>
  ...
  return (
    <div>
      <div style={{fontSize:12,color:'var(--t2)',marginBottom:14,padding:'10px 14px',borderRadius:11,background:'rgba(59,142,240,.06)',border:'1px solid rgba(59,142,240,.15)',lineHeight:1.5}}>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              fontSize: 13,
              fontWeight: 700,
              background: section === t.id ? (t.id === 'flash' ? 'rgba(255,69,69,.12)' : 'rgba(59,142,240,.12)') : 'var(--l2)',
              border: `1.5px solid ${section === t.id ? (t.id === 'flash' ? 'rgba(255,69,69,.35)' : 'rgba(59,142,240,.35)') : 'var(--b1)'}`,
              color: section === t.id ? (t.id === 'flash' ? '#FF8C8C' : '#5B9CF5') : 'var(--t2)',
            }}
          >
            {t.label}
      ...
      {loading ? (
        <div style={{padding:24,textAlign:'center',color:'var(--t2)'}}>Загрузка…</div>
      ) : section === 'flash' ? (
        <>
      <div style={{display:'flex',justifyContent:'flex-end',marginBottom:12}}>
          ...
          {flashPromos.length === 0 ? (
            <div style={{padding:'32px 16px',textAlign:'center',color:'var(--t3)',fontSize:13,background:'var(--l2)',borderRadius:12,border:'1px solid var(--b1)'}}>
        ...
        <>
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
            ...
            {(() => {
              const vis = ADMIN_CAT_VISUAL[activeCat.id] || { bg: 'var(--b1)', color: '#1FD760' }
              return (
                <>
              ...
              <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:11,color:'var(--t2)'}}>{activeCatItems.length} акционных товаров</div>
                </div>
        ...
        <>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:14}}>
            <div style={{fontSize:11,color:'var(--t2)',flex:1}}>Категории появляются автоматически, когда добавляете скидку на товар</div>
          ...
          {promosByCategory.length === 0 ? (
            <div style={{padding:32,textAlign:'center',color:'var(--t3)',fontSize:13,background:'var(--l2)',borderRadius:12,border:'1px solid var(--b1)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
              {promosByCategory.map(({ cat, items, maxDisc }) => {
                const vis = ADMIN_CAT_VISUAL[cat.id] || { bg: 'var(--b1)', color: '#1FD760' }
...
function PushPreview({ title, body }: { title: string; body: string }) {
  return (
    <div style={{
      padding: '14px 16px',
      borderRadius: 16,
      background: 'linear-gradient(145deg,var(--l1),var(--l3))',
      border: '1px solid var(--b1)',
      boxShadow: '0 8px 28px rgba(0,0,0,.35)',
    }}>
      <div style={{ fontSize: 9, color: 'var(--t3)', marginBottom: 10, fontWeight: 800, letterSpacing: 1 }}>ПРЕДПРОСМОТР · iOS / Android</div>
      <div style={{ display: 'flex', gap: 11, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 3, color: 'var(--t1)' }}>{title || 'Заголовок'}</div>
          <div style={{ fontSize: 12, color: 'var(--t2)', lineHeight: 1.5 }}>{body || 'Текст уведомления'}</div>
          <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 6 }}>КАКАПО · сейчас</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        background: on ? '#1FD760' : 'var(--b2)', position: 'relative', flexShrink: 0, transition: 'background .2s',
      }}
    >
      <div style={{
...
function PushPage() {
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        {[
          { l: 'Клиентов в базе', v: stats.totalClients, c: 'var(--t1)', e: '👥' },
          ...
          <div className="ac" style={{ padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 4 }}>Push попадёт в приложение клиента КАКАПО</div>
          </div>
              <Badge v={`${selectedRecipients.length} чел.`} c="#1FD760" />
            </div>

            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 8, fontWeight: 700 }}>Кому отправить</div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        background: active ? 'rgba(31,215,96,.1)' : 'var(--l3)',
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: active ? '#1FD760' : 'var(--t1)' }}>{seg.label}</span>
                      </div>
                      <div style={{ fontSize: 10, color: 'var(--t3)', lineHeight: 1.4 }}>{seg.hint}</div>
                      <div style={{ fontSize: 11, color: active ? '#1FD760' : 'var(--t2)', fontWeight: 800, marginTop: 6 }}>{n.toLocaleString()} чел.</div>
          </button>
                  );
                })}
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 5, fontWeight: 700 }}>Текст *</div>
                <textarea
                  value={body}
                  onChange={e => setBody(e.target.value)}
                  placeholder="Скидки до 40% только сегодня!"
                  style={{
                    background: 'var(--l3)', border: '1.5px solid var(--b1)', borderRadius: 10, color: 'var(--t1)',
              ...
              {templates.map(tpl => (
                <button
                  ...
                  style={{
                    ...
                    background: 'var(--l3)', border: '1px solid var(--b1)', cursor: 'pointer', textAlign: 'left', width: '100%',
                  }}
                >
                  <span style={{ fontSize: 22, flexShrink: 0 }}>{tpl.emoji}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--t1)' }}>{tpl.title}</div>
                    <div style={{ fontSize: 10, color: 'var(--t3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 2 }}>{tpl.body}</div>
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="ac" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 10, color: 'var(--t3)' }}>Связаны с заказами и картами</span>
            </div>
            {autoSettings.map((row, i) => (
              <div
                ...
                style={{
                  ...
                  borderBottom: i < autoSettings.length - 1 ? '1px solid var(--b1)' : 'none',
                }}
              >
                <span style={{ fontSize: 18, flexShrink: 0 }}>{row.icon}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: row.enabled ? 'var(--t1)' : 'var(--t2)' }}>{row.label}</div>
                  <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 2 }}>{row.description}</div>
                </div>
          ...
          <div className="ac" style={{ overflow: 'hidden' }}>
            <div style={{ padding: '13px 16px', borderBottom: '1px solid var(--b1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
              <div className="ub" style={{ fontSize: 13, fontWeight: 800 }}>📋 История</div>
              <span style={{ fontSize: 10, color: 'var(--t3)' }}>{history.length} рассылок</span>
            </div>
            ...
            {filteredHistory.length === 0 ? (
              ...
              return (
                <div key={h.id} style={{ padding: '12px 16px', borderBottom: i < filteredHistory.length - 1 ? '1px solid var(--b1)' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--t3)', flexShrink: 0 }}>{formatPushTime(h.sentAt)}</span>
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 8, lineHeight: 1.45 }}>{h.body}</div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--t2)' }}>{h.delivered.toLocaleString()} доставлено</span>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {FINANCE_TAB_OPTIONS.map(t => (
          <button key={t.id} type="button" onClick={() => setTab(t.id)} className="ab"
            style={{ padding: '8px 14px', fontSize: 12, background: tab === t.id ? 'rgba(31,215,96,.12)' : 'var(--l3)', border: `1.5px solid ${tab === t.id ? 'rgba(31,215,96,.35)' : 'var(--b1)'}`, color: tab === t.id ? '#1FD760' : 'var(--t2)', display: 'flex', alignItems: 'center', gap: 6 }}>
            {t.icon} {t.label}
          </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {[{ c: '#1FD760', l: 'Магазин' }, { c: '#FF8C00', l: 'Рестораны' }].map((l, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--t2)' }}>{l.l}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 5, alignItems: 'flex-end', height: 120 }}>
            {summary.dailyChart.map((d, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                ...
                <div style={{ fontSize: 9, color: 'var(--t3)' }}>{d.label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          {tab === 'restaurants' && (
            <div className="ac" style={{ padding: 16 }}>
              ...
              {summary.restaurants.filter(r => r.balance.pendingNet > 0).map(r => (
                <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--b1)' }}>
              ...
              {!summary.restaurants.some(r => r.balance.pendingNet > 0) && (
                <div style={{ fontSize: 12, color: 'var(--t3)', padding: '12px 0' }}>Все выплаты закрыты ✓</div>
              )}
          </div>
          )}
          ...
          {tab === 'couriers' && (
            <div className="ac" style={{ padding: 16 }}>
              ...
              {summary.couriers.slice(0, 5).map(c => (
                <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--b1)' }}>
          ...
          {tab === 'assemblers' && (
            <div className="ac" style={{ padding: 16 }}>
              ...
              {summary.assemblers.slice(0, 5).map(a => (
                <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--b1)' }}>
          ...
          {tab === 'shop' && (
            <div className="ac" style={{ padding: 16 }}>
              ...
              {[
                ...
                <Link key={l.href} href={l.href} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--b1)', fontSize: 12, color: 'var(--t2)', textDecoration: 'none' }}>
      ...
      {summary.recentPayouts.length > 0 && (
        <div className="ac">
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--b1)', fontWeight: 800, fontSize: 13 }}>История выплат ресторанам</div>
          <table className="at">
            ...
            <tbody>{summary.recentPayouts.slice(0, 8).map(p => (
              <tr key={p.id}>
                <td style={{ fontSize: 12, color: 'var(--t2)' }}>{p.date}</td>
          ...
          <div className="ac" style={{ width: '100%', maxWidth: 420, padding: 24 }} onClick={e => e.stopPropagation()}>
            {payDone ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--t2)', marginBottom: 20 }}>{payTarget.name} · {formatSm(restaurantBalance(payTarget).pendingNet)}</div>
              ...
              <>
                ...
                <div style={{ fontSize: 13, color: 'var(--t2)', marginBottom: 16 }}>{payTarget.emoji} {payTarget.name}</div>
                <div style={{ padding: 14, borderRadius: 12, background: 'rgba(255,184,0,.08)', border: '1px solid rgba(255,184,0,.2)', marginBottom: 16, textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: 'var(--t2)' }}>К выплате (нетто)</div>
                  <div className="ub" style={{ fontSize: 28, fontWeight: 900, color: '#FFB800' }}>{formatSm(restaurantBalance(payTarget).pendingNet)}</div>
                  <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 4 }}>Комиссия КАКАПО: {formatSm(restaurantBalance(payTarget).pendingCommission)}</div>
                </div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                  {[{ id: 'cash', l: '💵 Наличные' }, { id: 'card', l: '💳 Карта' }, { id: 'transfer', l: '🏦 Перевод' }].map(m => (
                    <button key={m.id} type="button" onClick={() => setPayMethod(m.id)} className="ab"
                      style={{ flex: 1, padding: 10, fontSize: 11, background: payMethod === m.id ? 'rgba(31,215,96,.12)' : 'var(--l3)', border: `1px solid ${payMethod === m.id ? 'rgba(31,215,96,.35)' : 'var(--b1)'}`, color: payMethod === m.id ? '#1FD760' : 'var(--t2)' }}>{m.l}</button>
                  ))}
                </div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button type="button" onClick={() => setPayTarget(null)} className="ab" style={{ flex: 1, padding: 12, background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)' }}>Отмена</button>
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
        {SETTINGS_QUICK_LINKS.map(link => (
          <button key={link.id} type="button" onClick={() => setPage(link.id)} className="btn ac"
            style={{ padding: '14px 12px', textAlign: 'left', cursor: 'pointer', background: 'var(--l2)', border: `1px solid ${link.color}28` }}>
            ...
            <div style={{ fontSize: 10, color: 'var(--t3)', lineHeight: 1.4 }}>{link.sub}</div>
          </button>
        ))}
      </div>
        ...
        {STABS.map(t => (
          <button key={t.id} type="button" onClick={() => setStab(t.id)} className="ab"
            style={{
              ...
              background: stab === t.id ? 'rgba(31,215,96,.12)' : 'var(--l3)',
              border: `1.5px solid ${stab === t.id ? 'rgba(31,215,96,.35)' : 'var(--b1)'}`,
              color: stab === t.id ? '#1FD760' : 'var(--t2)',
            }}>
            {t.l}
          </button>
        ))}
      ...
      {stab==='sms'&&(
        <div className="ac" style={{padding:20,maxWidth:520}}>
          ...
          <div style={{fontSize:11,color:'var(--t3)',marginBottom:12}}>Для входа курьеров, сборщиков и партнёров. Тариф доставки — в разделе «Тариф доставки».</div>
          ...
          <div style={{display:'flex',gap:8,marginBottom:14}}>
            {[{id:'smspro',l:'🇹🇯 SmsPro.tj'},{id:'eskiz',l:'🇺🇿 Eskiz'},{id:'twilio',l:'🌍 Twilio'}].map(p=><button key={p.id} type="button" onClick={()=>setSmsP(p.id)} className="ab" style={{flex:1,padding:'9px 6px',fontSize:12,background:smsP===p.id?'rgba(31,215,96,.12)':'var(--l3)',border:`1.5px solid ${smsP===p.id?'rgba(31,215,96,.35)':'var(--b1)'}`,color:smsP===p.id?'#1FD760':'var(--t2)'}}>{p.l}</button>)}
          </div>
          <NI lbl="API ключ" val={smsKey} set={setSmsKey} ph="Вставь ключ от провайдера" type="password"/>
          <div style={{marginTop:12,padding:'10px 13px',borderRadius:10,background:'rgba(59,142,240,.06)',border:'1px solid rgba(59,142,240,.2)',fontSize:12,color:'var(--t2)'}}>Для Таджикистана рекомендуем <span style={{color:'#3B8EF0',fontWeight:700}}>SmsPro.tj</span> — поддержка русского и таджикского языка</div>
        </div>
      )}
      ...
      {stab==='store'&&(
        <div className="ac" style={{padding:20,maxWidth:520}}>
          ...
          <div style={{fontSize:11,color:'var(--t3)',marginBottom:16}}>Отображаются клиентам. Адреса точек забора — в разделе «Точки забора».</div>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div className="ac" style={{padding:16}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {[{e:'🍽',l:'Добавить ресторан',a:'partners'},{e:'💸',l:'Создать акцию',a:'promos'},{e:'🔔',l:'Push рассылка',a:'push'},{e:'💰',l:'Выплаты',a:'finance'}].map((a,i)=>(
                <button key={i} onClick={()=>setPage(a.a)} className="ab" style={{padding:'10px 6px',background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',display:'flex',flexDirection:'column',alignItems:'center',gap:5,fontSize:11,fontFamily:'Nunito',fontWeight:700,lineHeight:1.4,textAlign:'center'}}>
  ...
  return (
    <div>
        ...
        {TARIFF_TAB_OPTIONS.map(opt => (
          <button key={opt.id} type="button" onClick={() => setTab(opt.id)} className="btn"
            style={{
              ...
              background: tab === opt.id ? `${opt.color}18` : 'var(--l3)',
              border: `1.5px solid ${tab === opt.id ? `${opt.color}55` : 'var(--b1)'}`,
              color: tab === opt.id ? opt.color : 'var(--t2)',
      ...
      <div style={{
        ...
        Доставка = <b style={{ color: 'var(--t1)' }}>База ({t.base} ЅМ)</b> + (км − {t.baseDist}) × <b style={{ color: 'var(--t1)' }}>{t.perKm} ЅМ</b>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 18, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
            {TARIFF_FIELD_META.map(f => (
              <div key={f.key} style={{
                background: 'linear-gradient(160deg,var(--l2),var(--l3))', border: '1px solid var(--b1)',
                borderRadius: 16, padding: '16px 18px', transition: 'border-color .2s',
              }}>
                <div style={{ fontSize: 11, color: 'var(--t2)', fontWeight: 700, marginBottom: 6 }}>{f.label}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--t3)', fontWeight: 700, whiteSpace: 'nowrap' }}>{f.unit}</span>
                </div>
                <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 8, lineHeight: 1.4 }}>{f.hint}</div>
              </div>
            ))}
          </div>

          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 11, color: 'var(--t2)', fontWeight: 700, marginBottom: 8 }}>⚡ Быстрые шаблоны</div>
            <div style={{ display: 'flex', gap: 10 }}>
              {TARIFF_PRESETS.map(p => (
                <button key={p.id} type="button" onClick={() => applyPreset(p.config)} className="btn"
                  style={{
                    ...
                    background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)',
                  }}>
                  <div style={{ fontSize: 18, marginBottom: 4 }}>{p.emoji}</div>
                  <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--t1)' }}>{p.label}</div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button type="button" onClick={resetDefaults} className="ab"
              style={{ flex: 1, padding: 12, background: 'var(--l3)', border: '1px solid var(--b1)', color: 'var(--t2)' }}>
              ↩️ Сбросить
            </button>
        ...
        {/* калькулятор */}
        <div style={{
          background: 'linear-gradient(160deg,rgba(31,215,96,.06),var(--l2))',
          ...
          <div style={{ fontSize: 11, color: 'var(--t3)', marginBottom: 16 }}>Проверьте тариф до сохранения</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
            {[
              ...
              <div key={f.lbl}>
                <div style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 4, fontWeight: 700 }}>{f.lbl}</div>
                <input type="number" className="ai" value={f.val} onChange={e => f.set(e.target.value)} placeholder={f.ph} />
              </div>
            ))}
          </div>

          <div style={{ background: 'var(--l1)', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
            {preview.breakdown.map((line, i) => (
              <div key={i} style={{ fontSize: 12, color: line.startsWith('✅') ? '#1FD760' : 'var(--t2)', marginBottom: 5 }}>{line}</div>
            ))}
            <div style={{
            ...
            {!preview.isFree && preview.feeOnly !== preview.full.total && (
              <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 6 }}>Без учёта суммы заказа: {preview.feeOnly} ЅМ</div>
            )}
          </div>

          {tab === 'couriers' && (
            <div style={{ padding: 12, borderRadius: 12, background: 'rgba(59,142,240,.08)', border: '1px solid rgba(59,142,240,.2)', fontSize: 12, color: 'var(--t2)' }}>
        ...
        <div className="ac" style={{ padding: 18 }}>
          ...
          {TAB_CONNECTIONS[tab].map(link => (
            <Link key={link.href} href={link.href}
              style={{ display: 'block', padding: '10px 0', borderBottom: '1px solid var(--b1)', textDecoration: 'none' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#1FD760' }}>{link.label} →</div>
              <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 2 }}>{link.desc}</div>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="ac">
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--b1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--t3)' }}>тариф: {t.base} + км × {t.perKm}</span>
        </div>
        <table className="at">
          <thead>
            <tr>
          ...
          <tbody>
            {previews.length === 0 && (
              <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--t3)', padding: 24 }}>Нет заказов для этой вкладки</td></tr>
            )}
            {previews.map(p => (
              <tr key={p.id}>
                ...
                <td>
                  ...
                  {p.locked && <div style={{ fontSize: 9, color: 'var(--t3)', marginTop: 2 }}>🔒 зафиксировано</div>}
                </td>
...
function PickupsPage() {
  ...
  const field = ({lbl,fld,ph='',type='text'}:{lbl:string,fld:string,ph?:string,type?:string}) => (
    <div><div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>{lbl}</div>
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <div style={{fontSize:12,color:'var(--t2)'}}>🏪 Добавляйте только магазины · рестораны — в «Рестораны»</div>
        <button onClick={openAdd} className="ab abp" style={{display:'flex',alignItems:'center',gap:6}}>+ Добавить магазин</button>
      </div>

      <div className="ac" style={{marginBottom:16}}>
        <div style={{padding:'10px 14px',borderBottom:'1px solid var(--b1)',fontWeight:800,fontSize:12}}>🏪 Магазины</div>
        <table className="at">
            ...
            {storeList.length ? storeList.map(p=>(
              <tr key={p.id}>
                <td><div style={{display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <div><div style={{fontWeight:700,fontSize:13}}>{p.name}</div><div style={{fontSize:10,color:'var(--t3)'}}>{p.id}</div></div>
                </div></td>
                ...
                <td style={{fontSize:12,color:'var(--t2)'}}>{p.addr}</td>
                <td style={{fontSize:12,color:'var(--t2)'}}>{p.phone}</td>
                <td style={{fontSize:11,color:'var(--t3)',fontFamily:'monospace'}}>{p.lat.toFixed(4)}, {p.lng.toFixed(4)}</td>
                <td>
                  <button onClick={()=>toggle(p.id)} className="ab" style={{padding:'4px 10px',fontSize:11,background:p.active?'rgba(31,215,96,.1)':'rgba(61,102,69,.1)',color:p.active?'#1FD760':'var(--t3)',border:`1px solid ${p.active?'rgba(31,215,96,.3)':'rgba(61,102,69,.3)'}`}}>
              ...
              <tr><td colSpan={7} style={{textAlign:'center',color:'var(--t3)',padding:20}}>Нет точек магазина</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="ac">
        <div style={{padding:'10px 14px',borderBottom:'1px solid var(--b1)',fontWeight:800,fontSize:12}}>🍽 Рестораны <span style={{fontWeight:400,color:'var(--t3)',fontSize:11}}>· из раздела «Рестораны»</span></div>
        <table className="at">
          <thead><tr><th>Точка</th><th>Тип</th><th>Адрес</th><th>Телефон</th><th>Координаты</th><th>Статус</th><th></th></tr></thead>
          <tbody>
            {restList.map(p=>(
              <tr key={p.id}>
                ...
                <td><span style={{fontSize:10,color:'var(--t3)'}}>из «Рестораны»</span></td>
              </tr>
            ))}
          </tbody>
        </table>
            ...
            {orders.map(o=>{
              ...
              return (
                <tr key={o.id}>
                  ...
                  <td>
                    <div style={{display:'flex',alignItems:'center',gap:4,flexWrap:'wrap'}}>
                      {o.pickupIds.map((pid,i)=>{
                        const pk = PM[pid] || { e:'📍', name: pid, color:'var(--t3)' };
                        ...
                        return (
                          <span key={i} style={{display:'inline-flex',alignItems:'center',gap:3,padding:'2px 7px',borderRadius:7,fontSize:11,fontWeight:700,
                            ...
                            color:isCur?pk.color:isDone?'#1FD760':'var(--t3)',
                            border:`1px solid ${isCur?pk.color+'55':isDone?'rgba(31,215,96,.3)':'var(--b1)'}`
                  ...
                  <td><div style={{fontSize:12,fontWeight:700}}>{o.client}</div><div style={{fontSize:10,color:'var(--t3)'}}>{o.addr}</div></td>
                  <td>
                  ...
                  <td>
                    ...
                    {o.step === 'done' && <span style={{ fontSize: 10, color: 'var(--t3)' }}> 🔒</span>}
                  </td>
                  <td>
                    <div style={{display:'flex',gap:3}}>
                      {o.pickupIds.map((_,i)=>(
                        <div key={i} style={{width:10,height:10,borderRadius:'50%',background:(o.step==='toClient'||o.step==='done'||(o.step==='toPickup'&&i<o.pickupIdx))?'#1FD760':(o.step==='toPickup'&&i===o.pickupIdx)?'#FFB800':'var(--b1)'}}/>
                      ))}
                      <div style={{width:10,height:10,borderRadius:'50%',background:o.step==='toClient'?'#9B6DFF':o.step==='done'?'#1FD760':'var(--b1)'}}/>
                    </div>
                  </td>
...
function BannersPage() {
  ...
  const field = ({label,val,onChange,type='text',half}:any) => (
    <div style={{marginBottom:12,flex:half?'1 1 48%':'1 1 100%'}}>
      <div style={{fontSize:11,color:'var(--t2)',marginBottom:4,fontWeight:700}}>{label}</div>
      <input className="ai" type={type} value={val} onChange={e=>onChange(e.target.value)}/>
    </div>
  );
  const Tog = ({on,onToggle}) => (
    <div onClick={onToggle} style={{width:42,height:23,borderRadius:12,background:on?'#1FD760':'var(--b1)',position:'relative',cursor:'pointer',transition:'background .2s',flexShrink:0}}>
  ...
  return (
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:20}}>
        {[{l:'Всего баннеров',v:banners.length,c:'var(--t1)'},{l:'Активных',v:banners.filter(b=>b.on).length,c:'#1FD760'},{l:'Скрытых',v:banners.filter(b=>!b.on).length,c:'var(--t2)'}].map((s,i)=>(
          <div key={i} className="ac" style={{padding:'14px 16px',textAlign:'center'}}>
            <div className="ub" style={{fontSize:24,fontWeight:900,color:s.c}}>{s.v}</div>
            <div style={{fontSize:11,color:'var(--t2)',marginTop:3}}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div className="ac" style={{padding:18,marginBottom:20}}>
        ...
        <div style={{background:'rgba(255,69,69,.08)',border:'1px solid rgba(255,69,69,.18)',borderRadius:10,padding:'7px 0',overflow:'hidden',marginBottom:14}}>
          <div style={{display:'flex',gap:0,whiteSpace:'nowrap',overflow:'hidden'}}>
            ...
            {tickers.filter(t=>t.on).length===0&&<span style={{fontSize:11,color:'var(--t3)',padding:'0 18px'}}>— нет активных элементов —</span>}
          </div>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:12}}>
          {tickers.map(t=>(
            <div key={t.id} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',background:'var(--l3)',borderRadius:10,border:`1px solid ${t.on?'var(--b1)':'rgba(255,69,69,.2)'}`,opacity:t.on?1:.65}}>
              <div onClick={()=>togTick(t.id)} style={{width:36,height:20,borderRadius:10,background:t.on?'#1FD760':'var(--b1)',position:'relative',cursor:'pointer',flexShrink:0,transition:'background .2s'}}>
      ...
      {showForm && (
        <div className="ac" style={{padding:20,marginBottom:20}}>
          ...
          <div style={{display:'flex',gap:8,marginTop:4}}>
            ...
            <button onClick={()=>{setShowForm(false);setEditId(null);setForm(DEF);}} className="ab" style={{padding:'9px 16px',background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)'}}>Отмена</button>
          </div>
        </div>
      )}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        {banners.map((b,idx)=>(
          <div key={b.id} className="ac" style={{overflow:'visible',border:`1.5px solid ${b.on?'var(--b1)':'rgba(255,69,69,.25)'}`,opacity:b.on?1:.75}}>
            ...
            <div style={{padding:'10px 14px',display:'flex',alignItems:'center',gap:8,background:'var(--l2)',borderRadius:'0 0 14px 14px'}}>
              <Tog on={b.on} onToggle={()=>toggle(b.id)}/>
              <span style={{fontSize:11,color:b.on?'#1FD760':'var(--t2)',fontWeight:700,flex:1}}>{b.on?'Активен':'Скрыт'}</span>
              <button onClick={()=>move(b.id,-1)} disabled={idx===0} className="ab" style={{padding:'5px 10px',fontSize:13,background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',opacity:idx===0?.3:1}}>↑</button>
              <button onClick={()=>move(b.id,1)} disabled={idx===banners.length-1} className="ab" style={{padding:'5px 10px',fontSize:13,background:'var(--l3)',border:'1px solid var(--b1)',color:'var(--t2)',opacity:idx===banners.length-1?.3:1}}>↓</button>
...
export default function AdminApp() {
  ...
  if (!ready) {
    return (
      <div style={{
        minHeight: '100vh', background: 'var(--bg)', color: 'var(--t2)',
...
function AdminAppInner({
  ...
  return (
    <Layout page={page} setPage={setPage} title={TITLES[page]||page} subtitle={SUBS[page]||''} session={session} onLogout={onLogout} theme={theme} onThemeChange={setTheme}>
      ...
      {page==='pospoints'  && <PosPointsAdminPage/>}
