...
import ProductsModule from '@/components/pos/ProductsModule'
import WarehouseModule from '@/components/pos/WarehouseModule'
...
const CSS = `
  ...
  .k-pos ::-webkit-scrollbar{width:8px;height:8px}
...
type PosPage = 'products' | 'warehouse'
...
function PosAppInner() {
  ...
  const current = (PAGES.some(p => p.id === page) ? page : 'products') as PosPage
      ...
      <div className="k-main">
        <header className="k-top">
          ...
          <div className="k-user">
            ...
            <div className="who"><b>Кассир</b><span>POS модуль</span></div>
          </div>
        </header>

        <div className="k-body">
          {!loaded ? (
            <div className="k-empty">Загрузка товаров…</div>
          ) : current === 'products' ? (
            <ProductsModule search={search} />
          ) : (
            <WarehouseModule products={products} />
          )}
        </div>
      </div>
    </div>
