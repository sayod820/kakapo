...
<#
.SYNOPSIS
  Собирает debug APK клиентского магазина КАКАПО (Capacitor).

.NOTES
  Нужны JDK 17+ и Android SDK.
  Ищет .tools рядом или в ../android-app/.tools
#>
...
function Find-JavaHome {
  ...
  $candidates = @(
    ...
    (Join-Path $Root '..\android-app\.tools\jdk-17'),
...
function Find-AndroidSdk {
  ...
  foreach ($bundled in @(
    ...
    (Join-Path $Root '..\android-app\.tools\android-sdk')
  )) {
...
Установите Android Studio / OpenJDK 17 или используйте android-app\.tools\jdk-17
'@ -ForegroundColor Red
  exit 1
}
if (-not $sdk) {
...
Запустите android-app\setup-android-sdk.ps1 или установите Android Studio.
'@ -ForegroundColor Red
  exit 1
}
...
if (-not (Test-Path (Join-Path $Root 'www\index.html'))) {
  Write-Host 'www missing: run npm run store:android:build-ui from kakapo root' -ForegroundColor Yellow
}
...
$outApk = Join-Path $outDir 'KAKAPO-Store-debug.apk'
